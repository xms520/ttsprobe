...
/* v4.5: 16000 → 24000 — QQ NT 语音 silk 是 24kHz 单声道(生态共识 NapCat/LLOneBot 同款)
 * 16k 数据被 QQ 24k 解码器解 → 变调+时长错乱 = "无法正常播放"主嫌疑之一 */
static NSInteger g_targetSampleRate = 24000;
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        /* 双候选 dump(不写入, 只 log 大小判决) */
        TTLog(@"[silk] 候选A(末次全量)=%luB 候选B(逐帧打包)=%luB",
              (unsigned long)lastFull.length, (unsigned long)acc.length);
        /* v4.6 默认走假设A: d 恒定(流式累计缓冲) → 输出 = 末次快照原样
         * 若日志判 dChanges>0 且 nAvg≈nMax → 改假设B */
        NSMutableData *body = (dChanges == 0 && lastN > 0 && lastFull.length == (NSUInteger)lastN)
                            ? [lastFull mutableCopy] : acc;
        TTLog(@"[silk] 采用=%@ body=%luB", (body == acc ? @"B(逐帧打包)" : @"A(末次全量)"),
...
- (void)sendDirect {
    ...
    void (^onAudio)(NSData *, NSError *) = ^(NSData *audio, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            TTLog(@"[qq] PCM %luB ≈ %lums — silk编码(24kHz)", (unsigned long)pcm.length, (unsigned long)ms);
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.6 init (callback 语义判决: 全序列dump+双候选A/B+方法签名dump)");
