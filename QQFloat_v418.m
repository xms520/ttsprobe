...
/* ===== v4.18: swizzle hook — 捕获 QQ 真实录音时的 codec 实例与调用参数 ===== */
static id g_realCodec = nil;          /* QQ 真实录音用的 QQSilkCodec 实例(strong 保留) */
static uint64_t g_realCount = 0;      /* QQ 真实调用的 count 参数(帧样本数? 判采样率!) */
...
static id QQEncodeHook(id self, SEL _cmd, id data, NSUInteger count, id cb) {
    if (!g_ttsEncoding && g_hookLogs < 4) {
        g_hookLogs++;
        g_realCodec = self;
        g_realCount = (uint64_t)count;
        NSData *d = (NSData *)data;
        const unsigned char *dp = d ? (const unsigned char*)d.bytes : NULL;
        NSUInteger dl = d ? d.length : 0;
        NSMutableString *hx = [NSMutableString string];
        if (dp) for (NSUInteger k = 0; k < dl && k < 16; k++) [hx appendFormat:@"%02x", dp[k]];
        TTLog(@"[codec-hook] self=%p count=%lu len=%lu head=%@",
              self, (unsigned long)count, (unsigned long)dl, hx);
    }
    if (g_origEncodeIMP)
        return ((id(*)(id,SEL,id,NSUInteger,id))g_origEncodeIMP)(self, _cmd, data, count, cb);
    return nil;
}
...
static void QQFloatV2Init(void) {
    ...
    InstallCodecHook();
    TTLog(@"QQFloat v4.18 init (codec-hook捕获QQ真实调用 + REAL/REC/OWN三路判决)");
