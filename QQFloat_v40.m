        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @try {
                ...
                if (!audioModel) { TTLog(@"[qq] class_createInstance 返回 nil"); }
                    ...
                    /* ---- P2: dlsym 桥 (v4.0: 全符号带 _ 前缀 + 四参 ABI) — 唯一首选 ----
                     * v3.9 崩因双实锤:
                     *   a) 无下划线 "$sSSMa"/"$sSS...FZ" 是 flat-lookup 垃圾命中
                     *      (0x185bb811c/0x1855cddf8 均非真符号) → metaFn() 即崩
                     *   b) NTSendable setAudioFilePath: 反汇编定案: 桥参数走 x2
                     *      (x0=String.Type, x2=NSString*, w3=1), v3.9 双参放 x1 → x2 垃圾 */
                    {
                        /* Mach-O dlsym 名必须带前导下划线 — v3.6~v3.9 的 tier-3
                         * 无下划线路径是垃圾命中根源, 判决性删除 */
                        ...
                        if (bridgeSym) {
                            ...
                            void *metaAcc = dlsym(RTLD_DEFAULT, "_$sSSMa");
                             ...
                             * q1 高 8 位非零(堆对象 0xD0/0x90 系) 或 q0 高字节 0xE0 系(small) */
                            BOOL plausible = (sq1 >> 56) != 0 || (sq0 >> 56) != 0;
                            if (!plausible) {
                                /* 兜底1: 单参直调(旧 ABI 尝试, 仅四参失败时) */
                                @try {
                    ...
                    /* ---- P3: small-string 手工布局 (≤15B ASCII) ---- */
                    if (!stringOK) {
                        ...
                        if (fl <= 15) {
                            ...
                            memcpy(buf, fp, fl);
                            /* Swift small-string: q0=前8B LE, q1 高字节=0xE0|count, 其余后 7B */
                            memcpy(&sq0, buf, 8);
                    ...
                    if (stringOK) {
                        ...
                        /* 唯一安全兜底: 空串 (0,0) — 不崩但大概率发不出;
                         * 等 D钩子 [qq-real] 位型实锤后 v3.9 硬编码 */
                        sq0 = 0; sq1 = 0;
                        const char *base = (const char *)(__bridge void *)audioModel;
                        memcpy((void *)(base + 0x10), &sq0, 8);
                        memcpy((void *)(base + 0x18), &sq1, 8);
                        TTLog(@"[qq] ⚠️ P1/P2/P3 全不成立 — 空串兜底(发送大概率失败,不崩)");
                    }
            ...
            @try {
                TTLog(@"[qq] sendPttMsg 调用前 (blocks=nil, model=%p)", (__bridge void*)audioModel);
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.0 init (四参桥 + 全符号带下划线 + P3/空串兜底)");
