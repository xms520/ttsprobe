/*
 * QQFloat_v32.m — v3.2: AudioModel 用 class_createInstance 绕 Swift 裸 init(闪退修复) + sendPttMsg attrs 传 nil
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @try {
                ...
                if (!audioModel) { TTLog(@"[qq] class_createInstance 返回 nil"); }
                    ...
                    /* dump ivar 布局（v4 修正字段用） */
                    unsigned ic = 0;
                    Ivar *ivs = class_copyIvarList(mCls, &ic);
                    if (ivs) {
                        for (unsigned k = 0; k < ic; k++) {
                            const char *iname = ivar_getName(ivs[k]);
                            TTLog(@"[qq-dump] AudioModel ivar %s@+%td", iname ? iname : "?",
                                  ivar_getOffset(ivs[k]));
                        }
                        free(ivs);
                    }
                    ...
                    TTLog(@"[qq] AudioModel 构造 ok (class_createInstance, 无init)");
                }
            } @catch (NSException *e) {
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v3.2 init (class_createInstance 绕 Swift init + attrs nil)");
    ...
    TTLog(@"[qq-init] v3.2 hooks installed");
}
@end
