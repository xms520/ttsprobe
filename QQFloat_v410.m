/*
 * QQFloat_v410.m — v4.10: v4.9 日志判决（QQFloat_2.log 44161B）→ 方法表全 dump 到手:
 *   ✅✅ QQSilkCodec 完整方法表(31个)暴露正确 API:
 *        encode:withSamplesCount:callback: [Q40@0:8@16Q24@?32]  ← 一直在用(绕callback的坑)
 *        encodeOneFrame:length:withSamplesCount: [@32@0:8^s16I24I28]  ← 单帧版
 *        encode:withSamplesCount: [@32@0:8^s16Q24]  ← 正确!返回 NSData 直接拿结果
 *        setSilkSampleRate: [v24@0:8Q16] / setSilkBitRate: / setSilkSamplesPerFrame: ← 盲区参数
 *   ❌ 根因: ①用错 encode API(绕 callback 语义猜了 4 版) ②setEncodeParam 无参没设码率/采样率
 *   ✅ QQSilkRecorder 方法表也到手: initWithSampleRate:andBitRate: [@32@0:8Q16Q24] 真正录音管线
 *
 * v4.10 改动:
 *   1. 弃 callback 版 → 用 encode:withSamplesCount: (^s16 + Q → NSData) 逐帧 20ms 编码拼接
 *   2. 正确设参: setSilkSampleRate:rate + setSilkSamplesPerFrame:rate/50 + setSilkBitRate:24000
 *
 * v4.10 判决树:
 *   - [silk] 帧0/1/2 len=XX head=... → 每帧 ~40B(16kbps) = 正常语音帧, 非 ~2B CNG
 *   - [silk] 逐帧编码 nFrames=.. totalOut=.. (~16kbps) → 编码正常=终审✅
 *   - 若 count 语义仍不对(每帧不是40B) → v4.11 换 QQSilkRecorder:initWithSampleRate:andBitRate:
 *
 * ====================== v4.9 判决存档 ======================
 * QQFloat_v49.m — v4.9: 方法表全 dump(诊断轮, 已达成目标)
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        /* v4.10: 弃用 callback 版 encode:withSamplesCount:callback:,
         * 改用 encode:withSamplesCount: (^s16 指针 + Q count → 返回 NSData 直接拿结果)
         * v4.9 方法表实锤: encode:withSamplesCount: [@32@0:8^s16Q24] 返回 @(NSData)
         * encodeOneFrame:length:withSamplesCount: [@32@0:8^s16I24I28] 单帧版
         * 关键: 逐帧 20ms 编码, 每帧返回 NSData 拼接成完整 silk */
        ...
        if ([codec respondsToSelector:spfSel]) ((void (*)(id, SEL, uint64_t))objc_msgSend)(codec, spfSel, (uint64_t)(rate/50)); /* 20ms 一帧 */
        if ([codec respondsToSelector:brSel]) ((void (*)(id, SEL, uint64_t))objc_msgSend)(codec, brSel, (uint64_t)24000); /* 24kbps */

        SEL encSel = NSSelectorFromString(@"encode:withSamplesCount:");  /* 非 callback 版 */
        if (![codec respondsToSelector:encSel]) {
            TTLog(@"[silk] encode:withSamplesCount: sel MISS (非callback版)");
            return nil;
        }
        const int16_t *samples = (const int16_t *)pcm.bytes;
        NSUInteger total = pcm.length / 2;           /* 总样本数 */
        NSUInteger frame = 20 * rate / 1000;         /* 20ms 一帧样本数 (16k→320) */
        __block NSMutableData *silk = [NSMutableData data];
        __block NSUInteger nFrames = 0;
        __block NSUInteger totalOut = 0;
        /* 逐帧编码: 每帧 320 样本, count=每帧样本数, 返回 NSData 拼接 */
        for (NSUInteger off = 0; off + frame <= total; off += frame) {
            @autoreleasepool {
                NSData *frameData = ((id (*)(id, SEL, const int16_t *, uint64_t))objc_msgSend)
                    (codec, encSel, samples + off, (uint64_t)frame);
                if (frameData && frameData.length > 0) {
                    [silk appendData:frameData];
                    totalOut += frameData.length;
                    if (nFrames < 3) {
                        const unsigned char *fp = frameData.bytes;
                        NSMutableString *hx = [NSMutableString string];
                        for (NSUInteger k = 0; k < frameData.length && k < 16; k++)
                            [hx appendFormat:@"%02x", fp[k]];
                        TTLog(@"[silk] 帧%lu len=%lu head=%@", (unsigned long)nFrames,
                              (unsigned long)frameData.length, hx);
                    }
                    nFrames++;
                }
            }
        }
        if (nFrames == 0) { TTLog(@"[silk] 无帧输出(total=%lu frame=%lu)", (unsigned long)total, (unsigned long)frame); return nil; }
        TTLog(@"[silk] 逐帧编码 nFrames=%lu totalOut=%luB (%.1f kbps)", (unsigned long)nFrames,
              (unsigned long)totalOut, (double)totalOut * 8 / ((double)total / rate));
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.10 init (encode:withSamplesCount: 非callback版逐帧编码 + 正确设码率采样率)");
