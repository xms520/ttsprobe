/*
 * QQFloat_v411.m — v4.11: v4.10 日志判决（QQFloat_3.log 37832B）→ 显示"编码失败":
 *   ❌ encode:withSamplesCount: 分帧(320样本)编码返回 nil → 无帧输出
 *      total=42229/66549/38389(真实语音 maxAbs=24907/17127/12083), frame=320
 *   ❌ 根因判断: encode:withSamplesCount: [@32@0:8^s16Q24] 的 count 语义 = 总样本数,
 *      需要一次性传完整 PCM 指针(不是分帧); 而且 init 链漏了 allocEncodeBuffer/initAgc/initNsx
 *
 * v4.11 改动:
 *   1. 补完整初始化链: setEncodeParam + setSilkSampleRate/bitRate/samplesPerFrame
 *      + allocEncodeBuffer + initAgc + initNsx(QQ 录音流程必走, 之前全漏)
 *   2. encode:withSamplesCount: 一次性传全部样本(count=总样本数)
 *
 * v4.11 判决树:
 *   - [silk] 一次性 encode ok total=.. → silk N B (~16kbps) → 编码正常=终审✅
 *   - 仍返回 nil/空 → v4.12 换 QQSilkRecorder:initWithSampleRate:andBitRate:(真正录音管线)
 *
 * ====================== v4.10 判决存档 ======================
 * QQFloat_v410.m — v4.10: 逐帧 encode:withSamplesCount: 返回 nil(分帧语义错)
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        /* v4.11: 补完整初始化链 + 一次性 encode:withSamplesCount: (^s16+总样本数→NSData)
         * v4.10 分帧(320样本)返回 nil → 该方法 count 语义=总样本数, 需一次性完整 PCM
         * 方法表盲区项: allocEncodeBuffer/initAgc/initNsx 之前全漏了(QQ 录音流程必走) */
        ...
        /* 一次性喂入全部样本: count=总样本数, data=完整 PCM 指针 */
        NSData *enc = ((id (*)(id, SEL, const int16_t *, uint64_t))objc_msgSend)
            (codec, encSel, samples, (uint64_t)total);
        if (!enc || enc.length == 0) {
            TTLog(@"[silk] 一次性 encode 返回 nil/空 (total=%lu rate=%u)", (unsigned long)total, rate);
            return nil;
        }
        TTLog(@"[silk] 一次性 encode ok total=%lu → silk %luB (%.1f kbps)",
              (unsigned long)total, (unsigned long)enc.length,
              (double)enc.length * 8 / ((double)total / rate));
        if (enc.length < 16) {
            const unsigned char *fp = enc.bytes;
            NSMutableString *hx = [NSMutableString string];
            for (NSUInteger k = 0; k < enc.length; k++) [hx appendFormat:@"%02x", fp[k]];
            TTLog(@"[silk] encode 输出头=%@", hx);
        }
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.11 init (补全 initAgc/Nsx/allocBuffer + encode 一次性 count=总样本数)");
