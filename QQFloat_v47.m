/*
 * QQFloat_v47.m — v4.7: v4.6 日志判决（QQFloat_3.log 32130B）→ TTS 仍不正常:
 *   ✅ callback 语义终审判决: n=当前输出缓冲累计字节数, d=缓冲指针
 *      铁证: nAvg=252=(1+504)/2 严格线性, nMax=504=cbs, dump 显示 d 内容逐次 +1B
 *      → 每次 callback 写 1 字节通知一次! 正确输出 = 末次全量缓冲原样(候选A)
 *   ✅ encode 方法签名: Q (@ Q @?) — (id, SEL, @data, Q count, @?block) 调用形态正确
 *   ❌ v4.6 走了候选B(嵌套累计 128268B 垃圾) — dChanges=14 触发误判, 已修
 *   ❌ 核心新发现: 24kHz 编码总输出仅 504B/3.36s ≈ 1.2kbps = silk CNG 静音帧率!
 *      QQ 真实录音 5757B/3s ≈ 15kbps — 差 13 倍
 *      → QQSilkCodec.setEncodeParam() 无参调用 = 默认 16kHz!
 *      24kHz 语音喂 16kHz codec → 能量被判高频噪声 → 全帧 CNG → 静音输出!
 *
 * v4.7 三处改动:
 *   1. g_targetSampleRate 24000 → 16000 回退(codec 默认判决; QQ 播放端按文件头自适应)
 *   2. 输出恒用候选A(末次全量) — 嵌套累计 B 候选永久删除
 *   3. PCM 振幅取证(maxAbs) — 判静音是"输入就静音"(Converter bug)还是"codec 判静音"
 *
 * v4.7 判决树:
 *   - [qq] PCM .. maxAbs>5000 → 输入正常
 *   - [silk] 候选A=nMax ≈ 16kbps×秒数(如 2s≈4000B) → 编码正常 → 播放正常=终审✅
 *   - 仍 ≈1.2kbps(CNG) → 16kHz 也不是 codec 默认 → v4.8 解 setEncodeParam 带参(采样率/码率)
 *   - maxAbs<100 → PCM 链 bug(AVAudioConverter), v4.8 重写 PCM 提取
 *   ⚠️ 副作用不变: 真实录音文件被覆写
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        NSUInteger frame = 20 * rate / 1000;   /* 20ms 一帧 (24k→480 样本) */
        /* v4.6: callback 全序列取证 — 判 d/n 语义
         * 假设A: d=同一缓冲, n=累计长度(流式进度) → 输出=末次 d[0..n] 全量
         * 假设B: d=每帧独立缓冲, n=帧长 → 输出=[u16 n][frame] 逐帧打包 */
        __block NSMutableData *acc = [NSMutableData data];          /* 假设B: 逐帧打包 */
        __block NSMutableData *lastFull = [NSMutableData data];     /* 假设A: 末次全量快照 */
        __block NSUInteger cbs = 0;          /* callback 次数 */
        __block const void *lastD = NULL; __block uint64_t lastN = 0;
        __block NSUInteger dChanges = 0;     /* d 指针变化次数(判 A/B) */
        __block uint64_t nSum = 0, nMax = 0;
        __block NSMutableString *dumpHead = [NSMutableString string]; /* 前 12 次 (d,n,head6B) */
        for (NSUInteger off = 0; off + frame <= total; off += frame) {
            ...
            ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
                (codec, encSel, chunk, frame, ^(const void *d, uint64_t n) {
                    ...
                    if (cbs < 12) {
                        unsigned char h[6] = {0};
                        memcpy(h, d, n < 6 ? (size_t)n : 6);
                        [dumpHead appendFormat:@"[%llu@%p=%02x%02x%02x%02x%02x%02x]",
                            (unsigned long long)n, d, h[0],h[1],h[2],h[3],h[4],h[5]];
                    }
                    if (lastD && d != lastD) dChanges++;
                    lastD = d; lastN = n;
                    nSum += n; if (n > nMax) nMax = n;
                    /* 假设A: 全量快照(每 callback 覆盖, 末次 = 完整输出) */
                    [lastFull setData:[NSData dataWithBytes:d length:(NSUInteger)n]];
                    /* 假设B: 逐帧打包 [u16 n][frame] */
                    uint16_t ln = (uint16_t)n;
                    [acc appendBytes:&ln length:2];
                    [acc appendBytes:d length:(NSUInteger)n];
                    cbs++;
                });
        }
        if (cbs == 0) { TTLog(@"[silk] 无帧输出"); return nil; }
        TTLog(@"[silk] cbs=%lu dChanges=%lu nMax=%llu nAvg=%llu dump=%@",
              (unsigned long)cbs, (unsigned long)dChanges,
              (unsigned long long)nMax, (unsigned long long)(nSum/cbs), dumpHead);
        /* v4.6 日志判决: n=当前缓冲累计字节数(每次 callback 写 1 字节通知一次)
         * d=输出缓冲(偶有重分配 dChanges 无害, 末次快照为准)
         * → 正确输出 = 候选A: 末次全量缓冲原样(不加任何前缀/打包!) */
        TTLog(@"[silk] 候选A(末次全量)=%luB 候选B(已弃)=%luB nMax=%llu",
              (unsigned long)lastFull.length, (unsigned long)acc.length,
              (unsigned long long)nMax);
        NSMutableData *body = [lastFull mutableCopy];
        if (body.length != (NSUInteger)nMax)
            TTLog(@"[silk] ⚠️ A长度(%lu)≠nMax(%llu) — 末次快照异常, 看 dump",
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.7 init (16kHz回退: codec CNG静音判决 + 候选A直出 + PCM振幅取证)");
