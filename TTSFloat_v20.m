...
/* TTS PCM 字节速率（16kHz × 2字节）。若实测发现日志里语音被拉长/压短，
 * 把 32000 改成 16000（微信录音器实际 8kHz 采集）再验证。 */
#define TTS_PCM_RATE 32000
static uint64_t g_aqLastNs = 0;      /* 上次回调时间戳 */
static uint32_t g_aqRateBps = 0;     /* 实测采集速率 B/s（0=未知） */
...
static void TTS_AQInputTrampoline(void *inUserData, AudioQueueRef inAQ,
    ...
    if (g_replaceActive && g_pendingPCM && inBuffer && inBuffer->mAudioData) {
        @synchronized([NSObject class]) {
            ...
            /* 用前 6 次回调的（buffer / 间隔）中值思想定速率：取第一个可靠值后保持 */
            if (dtNs > 2 * 1000 * 1000 && dtNs < 2000LL * 1000 * 1000) { /* 2ms~2s */
                uint32_t r = (uint32_t)((uint64_t)bufSz * 1000000000ULL / dtNs);
                if (r >= 1000 && r <= 4000000) {
                    if (g_aqRateBps == 0 || g_aqCbSeq <= 6) g_aqRateBps = r;
                }
            }
            uint32_t rate = g_aqRateBps ? g_aqRateBps : TTS_PCM_RATE;

            NSUInteger total = g_pendingPCM.length;
            NSUInteger take = 0;
            if (g_pcmOffset < total) {
                if (dtNs == 0) {
                    take = (NSUInteger)(rate / 50);   /* 首帧按 20ms 起步 */
                } else {
                    take = (NSUInteger)((uint64_t)rate * dtNs / 1000000000ULL);
                }
                if (take == 0) take = 64;
                if (take > total - g_pcmOffset) take = total - g_pcmOffset;
            ...
            /* v22 诊断：每次回调都记录（seq/size/dt/est-rate/fed/total） */
            TTLog(@"[aq-cb] #%lu size=%u dt=%llums rate=%uB/s fed=%lu/%lu",
                  (unsigned long)g_aqCbSeq, bufSz,
                  (unsigned long long)(dtNs / 1000000ULL), rate,
...
static BOOL TTSSafeOneArgMethod(Class cls, NSString *name) {
    Method m = class_getInstanceMethod(cls, NSSelectorFromString(name));
    if (!m) return NO;
    const char *t = method_getTypeEncoding(m);
    /* 仅接受 B/v/@ 返回 + 单对象参（24 字节栈：@0:8@16） */
    if (!t) return NO;
    char r = t[0];
    if (r != 'B' && r != 'v' && r != '@') return NO;
    return strstr(t, "@0:8@16") != NULL;
}
...
static void InstallRecorderEndCapture(void) {
    ...
    dispatch_once(&once, ^{
        ...
        NSArray *cands = @[ @"OnRecorderEndRecording:",
                            @"OnRecorderEndRecording:UserData:",
                            @"OnRecorderEndRecording:Error:" ];
...
- (void)v22_finishRecordAndWait:(int)waitedMs;
@end
    ...
    RequestTTS(text, voice, ^(NSData *audio, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @synchronized([NSObject class]) {
                ...
                g_replaceActive = YES;
                g_pcmFedDone = NO;
                g_aqLastNs = 0; g_aqRateBps = 0; g_aqCbSeq = 0; g_aqStartNs = 0;  /* v22 节奏状态复位 */
            }
            ...
            if (recording) {
                ...
                dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self v22_finishRecordAndWait:waitedCopy];
                    });
                });
            } else {
...
/* ==================== v22 收尾（全部主线程） ====================
 * ⚠️ v21 在 global_queue 后台线程直接调 StopRecord / SendOriVoiceMsgWithUserData:
 * ——这两个方法都会走 UI（气泡）与消息队列，必须在主线程。
 * v22：StopRecord → 0.6s（录音器收尾 silk 分片提交）→ 主线程复刻真实发送入口
 *      → 2.5s 后回检是否已走到 prepareSend（= 发送链确实启动） */
- (void)v22_finishRecordAndWait:(int)waitedMs {
    id audioSender = nil;
    @synchronized([NSObject class]) { audioSender = g_audioSender; }
    if (!audioSender) { TTLog(@"[v22] 无 AudioSender"); return; }

    SEL stopSel = NSSelectorFromString(@"StopRecord");
    @try {
        ((void (*)(id, SEL))objc_msgSend)(audioSender, stopSel);
        TTLog(@"[v22] StopRecord done (waited=%dms) 主线程", waitedMs);
    } @catch (NSException *e) { TTLog(@"[v22] StopRecord 异常: %@", e); }

    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.6 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        id endUD = nil, startUD = nil, sender = nil;
        NSString *endSelName = nil;
        @synchronized([NSObject class]) {
            endUD = g_realEndUserData;      /* 微信真实结束时用的 userData */
            startUD = g_lastUserInfoParam;  /* 面板编程式启动传的 userInfo */
            sender = g_audioSender;
            endSelName = g_realEndSelector;
        }
        int seen = g_recorderEndSeen;
        TTLog(@"[v22] 诊断 endSeen=%d endSel=%@ endUD=%p startUD=%p",
              seen, endSelName ?: @"-", (__bridge void *)endUD, (__bridge void *)startUD);
        if ([sender respondsToSelector:NSSelectorFromString(@"OnRecorderEndRecording:")]) {
            TTLog(@"[v22] StopRecord 后微信自己已跑到 OnRecorderEndRecording: → 无需复刻，等上传");
        }

        BOOL sent = NO;
        /* ⚠️ 若微信自己已经跑到结束回调（StopRecord 触发），说明发送已由真实链启动；
         * 此时再复刻一次会发重复消息 —— 只在真实链 1.2s 内没有新动作时才兜底触发。 */
        BOOL realChainRan = (g_recorderEndSeen > seen) ||
                            (g_recorderEndTime > 0 &&
                             ([NSDate date].timeIntervalSinceReferenceDate - g_recorderEndTime) < 1.2);
        if (realChainRan) {
            sent = YES;
            TTLog(@"[v22] 真实结束链已跑（endSeen %d→%d）— 跳过复刻防重复发送", seen, g_recorderEndSeen);
        }
        @try {
            if (endUD) {   /* 优先级①：微信真实结束时抓到的 userData */
                SEL s = NSSelectorFromString(@"SendOriVoiceMsgWithUserData:");
                if ([sender respondsToSelector:s]) {
                    BOOL (*fn)(id, SEL, id) = (BOOL (*)(id, SEL, id))objc_msgSend;
                    sent = fn(sender, s, endUD);
                    TTLog(@"[v22] SendOri(realUD) ret=%d", sent);
                }
            }
            if (!sent && startUD) {   /* 优先级②：启动参数字典（v21 原做法，兜底） */
                SEL s = NSSelectorFromString(@"SendOriVoiceMsgWithUserData:");
                if ([sender respondsToSelector:s]) {
                    BOOL (*fn)(id, SEL, id) = (BOOL (*)(id, SEL, id))objc_msgSend;
                    sent = fn(sender, s, startUD);
                    TTLog(@"[v22] SendOri(startUD) ret=%d", sent);
                }
            }
            if (!sent && endUD && endSelName.length) { /* 优先级③：复刻真实结束回调本身 */
                SEL s = NSSelectorFromString(endSelName);
                if ([sender respondsToSelector:s] &&
                    TTSSafeOneArgMethod([sender class], endSelName)) {
                    ((id (*)(id, SEL, id))objc_msgSend)(sender, s, endUD);
                    TTLog(@"[v22] 复刻真实结束回调 %@ 已执行", endSelName);
                }
            }
        } @catch (NSException *e) { TTLog(@"[v22] 发送入口异常: %@", e); }
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.5 * NSEC_PER_SEC)),
            ...
            TTLog(@"[v22] 收尾完成 sent=%d — 若气泡仍转圈，看 [part]/[capture] 是否出现", sent);
            self.send.enabled = YES; [self.spinner stopAnimating];
            self.statusLabel.text = sent ? @"✅ 已发送" : @"⚠️ 已停止录音（发送入口未确认）";
            if (sent) self.input.text = @"";
        });
    });
}
...
static void TTSShowBall(void) {
    ...
    TTLog(@"===== v22 ball shown（实时节奏喂数 + 主线程真实发送入口复刻） =====");
}
...
static void TTSFloatV14Init(void) {
    ...
    TTLog(@"v22 init (实时节奏喂数 + 主线程发送入口复刻)");
}
