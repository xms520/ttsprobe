        ...
        if (types && types[0] == 'B') {
            IMP newImp = imp_implementationWithBlock(^BOOL(id self, id arg) {
                @try {
                    ...
                    g_prepareSendSeen++;
                    TTLog(@"[capture] prepareSend: #%d self=%p userData=%p ret=?",
                          g_prepareSendSeen, (__bridge void *)self, (__bridge void *)arg);
                    if (arg) {
...
- (void)v23_finishRecordAndWait:(int)waitedMs;
@end
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            if (recording) {
                ...
                dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self v23_finishRecordAndWait:waitedCopy];
                    });
                });
            } else {
...
- (void)v23_finishRecordAndWait:(int)waitedMs {
    ...
    if (!audioSender) { TTLog(@"[v23] 无 AudioSender"); return; }
    ...
    @try {
        ...
        TTLog(@"[v23] StopRecord done (waited=%dms) 主线程 endSeen=%d prepSeen=%d",
              waitedMs, seenBefore, prepBefore);
    } @catch (NSException *e) { TTLog(@"[v23] StopRecord 异常: %@", e); }
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.5 * NSEC_PER_SEC)),
        ...
        TTLog(@"[v23] 收尾 end:%d→%d prepareSend:%d→%d %@",
              seenBefore, g_recorderEndSeen, prepBefore, g_prepareSendSeen,
              prepFired ? @"（发送链已由真实流程启动）" : @"（未观察到 prepareSend → 真实链没走到发送）");
        self.send.enabled = YES; [self.spinner stopAnimating];
        self.statusLabel.text = prepFired ? @"✅ 已发送" : @"⚠️ 未触发发送链（看日志）";
        if (prepFired) self.input.text = @"";
    });
}
...
static void TTSShowBall(void) {
    ...
    TTLog(@"===== v23 ball shown（整块填满 + 真实链自动发送，不再回传 userData） =====");
}
...
static void TTSFloatV14Init(void) {
    ...
    TTLog(@"v23 init (整块填满修复杂音 + 移除危险 userData 回传修复闪退)");
