/*
 * QQFloat_v33.m — v3.3: KVC→object_setIvar 直写(Swift类KVC不compliant实测) + 补写 audioDuration
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @try {
                ...
                if (!audioModel) { TTLog(@"[qq] class_createInstance 返回 nil"); }
                    ...
                    /* v3.3: KVC 对 Swift 类抛 not-KVC-compliant（v3.2 实测），
                     * 改用 object_setIvar 直接按 ivar 写入 —— 零方法查找、零异常路径 */
                    ...
                    if (!ivPath || !ivType) {
                        TTLog(@"[qq] AudioModel 关键 ivar MISS path=%p type=%p", ivPath, ivType);
                    } else {
                        /* audioFilePath: NSString* */
                        object_setIvar(audioModel, ivPath, silkPath);
                        /* audioType: 枚举(底层 uint) —— 1=silk 假设【推测，按 sendResult 判决修正】 */
                        object_setIvar(audioModel, ivType, @(1));
                        /* audioDuration: 毫秒时长（v3.2 dump 实证存在此字段） */
                        ...
                        TTLog(@"[qq] AudioModel 构造 ok (object_setIvar 直写, ct=1 dur=%lu)",
                              (unsigned long)ms);
                    }
                }
            } @catch (NSException *e) {
            ...
            /* v3.2: msgAttributeInfos 传 nil（msgAttrs 可能是捕获的旧 attrs, 含过期 msgId 反而干扰 NT kernel）
             * sendPttMsg 内部自己组装 attributes —— placeholderMsgInfo 也 nil */
            @try {
                ((void (*)(id, SEL, id, id, id, void (^)(BOOL), void (^)(int, NSString *)))objc_msgSend)
                    (handler, sendSel, audioModel, nil, nil, saveBlk, sendBlk);
                TTLog(@"[qq] sendPttMsg 已调用 (attrs=nil)");
            } @catch (NSException *e) {
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v3.3 init (object_setIvar 直写 + 全字段)");
    ...
    TTLog(@"[qq-init] v3.3 hooks installed");
}
@end
