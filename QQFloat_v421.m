...
- (void)injectTapped {
    ...
    self.statusLabel.text = [NSString stringWithFormat:@"注入待命 %.1f 秒：去聊天按住说话再松手", sec];
    TTLog(@"[inject] on: %luB @%lluHz ≈ %.2fs (PCM 源 %luB @%ld)",
          (unsigned long)conv.length, sr, sec, (unsigned long)pcm.length, (long)g_targetSampleRate);
}
...
- (void)sendDirect {
    ...
    void (^onAudio)(NSData *, NSError *) = ^(NSData *audio, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            {
                uint64_t srUse = g_realSampleRate ? g_realSampleRate : 16000;
                NSData *conv = ResamplePCM16(pcm, (double)g_targetSampleRate, (double)srUse);
                g_injectPcm = conv;
                g_injectTotal = conv.length;
                g_injectPos = 0;
                g_injectTail = 0;
                g_injectActive = YES;
                double sec = (double)conv.length / 2.0 / (double)srUse;
                TTLog(@"[qq] v4.21 合成完成 → 自动注入待命: %luB @%lluHz ≈ %.2fs",
                      (unsigned long)conv.length, srUse, sec);
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.send.enabled = YES; [self.spinner stopAnimating];
...
static void QQFloatV2Init(void) {
    ...
    InstallCodecHook();
    InstallProbeHooks();
    TTLog(@"QQFloat v4.21 init (合成→自动注入待命, 弃用自编码静音链, cleanEncode 探针)");
