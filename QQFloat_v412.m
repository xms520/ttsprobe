/*
 * QQFloat_v412.m — v4.12: v4.11 日志判决（QQFloat_4.log 36940B）→ 仍"编码失败":
 *   ❌ encode:withSamplesCount: 一次性(count=总样本数28149)仍返回 nil
 *   → count 语义可能=字节数, 或该方法依赖文件流上下文(openSilkFile 等)
 *   方法表铁证: encodeOneFrame:length:withSamplesCount: [@32@0:8^s16I24I28]
 *     = (int16* data, unsigned length字节数, unsigned samplesCount样本数) 返回 NSData
 *
 * v4.12 双路径并行(日志判死):
 *   路径A: encodeOneFrame:length:withSamplesCount: 逐帧(length=frame*2, samplesCount=frame)
 *   路径B: encode:withSamplesCount: 传 count=字节数(pcm.length)
 *
 * v4.12 判决树:
 *   - [silk] EOF帧0/1 len=XX head=... ~40B = 正常语音帧 → 终审✅
 *   - [silk] 路径B(count=字节数) ok → 语义定案=字节数
 *   - 两路径都空 → v4.13 换 QQSilkRecorder(openPcmFile/openSilkFile 完整管线)
 *
 * ====================== v4.11 判决存档 ======================
 * QQFloat_v411.m — v4.11: 一次性 encode(count=总样本数) 返回 nil(仍失败)
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        /* v4.12: 换 encodeOneFrame:length:withSamplesCount: (最明确接口, 分 length 和 samplesCount)
         * v4.11 一次性 encode:withSamplesCount: (count=总样本数 28149) 仍返回 nil
         * → 该方法的 count 可能=字节数, 或需要文件流上下文(openSilkFile 等)
         * 方法表铁证: encodeOneFrame:length:withSamplesCount: [@32@0:8^s16I24I28]
         *   = (data指针, length字节数, samplesCount样本数) 返回 NSData — 单帧编码入口 */
        ...
        /* 路径A: encodeOneFrame:length:withSamplesCount: 逐帧编码 */
        SEL eofSel = NSSelectorFromString(@"encodeOneFrame:length:withSamplesCount:");
        BOOL hasEOF = [codec respondsToSelector:eofSel];
        NSMutableData *silk = [NSMutableData data];
        if (hasEOF) {
            NSUInteger nFrames = 0, totalOut = 0;
            for (NSUInteger off = 0; off + frame <= total; off += frame) {
                @autoreleasepool {
                    NSData *fd = ((id (*)(id, SEL, const int16_t *, unsigned int, unsigned int))objc_msgSend)
                        (codec, eofSel, samples + off, (unsigned int)(frame*2), (unsigned int)frame);
                    if (fd && fd.length > 0) {
                        [silk appendData:fd]; totalOut += fd.length;
                        if (nFrames < 2) {
                            const unsigned char *fp = fd.bytes;
                            NSMutableString *hx = [NSMutableString string];
                            for (NSUInteger k = 0; k < fd.length && k < 16; k++) [hx appendFormat:@"%02x", fp[k]];
                            TTLog(@"[silk] EOF帧%lu len=%lu head=%@", (unsigned long)nFrames, (unsigned long)fd.length, hx);
                        }
                        nFrames++;
                    }
                }
            }
            TTLog(@"[silk] encodeOneFrame 逐帧: nFrames=%lu totalOut=%luB", (unsigned long)nFrames, (unsigned long)totalOut);
            if (nFrames == 0) { TTLog(@"[silk] EOF 逐帧也返回空 — 换路径B"); }
        } else {
            TTLog(@"[silk] encodeOneFrame sel MISS");
        }

        /* 路径B: encode:withSamplesCount: 传 count=字节数(=pcm.length) */
        SEL encSel = NSSelectorFromString(@"encode:withSamplesCount:");
        NSData *encB = nil;
        if ([codec respondsToSelector:encSel]) {
            encB = ((id (*)(id, SEL, const int16_t *, uint64_t))objc_msgSend)
                (codec, encSel, samples, (uint64_t)pcm.length);   /* count=字节数 */
            TTLog(@"[silk] 路径B(count=字节数%lu) → %s %luB",
                  (unsigned long)pcm.length, (encB ? "ok" : "nil"), (unsigned long)(encB ? encB.length : 0));
        }

        /* 选择输出: 优先路径A(多帧); 若空再用路径B */
        NSData *outputData = nil;
        if (silk.length > 0) outputData = silk;
        else if (encB && encB.length > 0) outputData = encB;

        if (!outputData || outputData.length == 0) {
            TTLog(@"[silk] 两路径都空 — 编码失败");
            return nil;
        }
        TTLog(@"[silk] 编码成功 silk=%luB (%.1f kbps)", (unsigned long)outputData.length,
              (double)outputData.length * 8 / ((double)total / rate));
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.12 init (encodeOneFrame 逐帧 + count=字节数双路径诊断)");
