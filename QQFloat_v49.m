/*
 * QQFloat_v49.m — v4.9: v4.8 日志判决（QQFloat_1.log 34093B）→ 语音仍 1 秒:
 *   ✅ v4.8 一次性喂全量 PCM: cbs=199/247/207, nMax=199/247/207, body=199/247/207B
 *      d 指针全程不变(如 0x285014820 恒定), PCM maxAbs=11888/10675/9945(真实语音)
 *   ❌ 关键铁证: dump 从 n=6 起恒 29f752f6a121(6字节) — 编码器只产出 6 字节固定内容
 *      cbs≈帧数(PCM 1999ms→199), 但每帧内容都是同样 6 字节 = silk CNG 静音帧
 *   ❌ 盲区: ①QQSilkCodec 完整方法表从没 dump 过(一直盲调 setEncodeParam/encode)
 *     ②setEncodeParam 无参调用没配好码率/采样率(编码器默认 CNG 静音)
 *
 * v4.9 三处改动(诊断向):
 *   1. QQSilkEncode 开头 dump QQSilkCodec 完整方法表(找正确 init/encode API)
 *   2. callback 取证增强: nFirst + 12B head + concat(全 callback 合并) + body(末次快照)
 *   3. init dump 类列表增加 QQSilkCodec/QQSilkRecorder/QQSilkEncodeDecode/AudioSilkCodec/SilkCodec/PcmEncoder
 *
 * v4.9 判决树:
 *   - [silk] QQSilkCodec 方法共 N 个 + 签名列表 → 找到正确 encode/init API(关键)
 *   - [silk] cbs/nFirst/nMax/body/concat → concat>1000B = 每帧独立拼接才对
 *   - [qq-dump] QQSilkRecorder 方法表 → 含 initWithSampleRate:andBitRate: 直接换此 API
 *
 * ====================== v4.8 判决存档 ======================
 * QQFloat_v48.m — v4.8: v4.7 日志判决 → encode count=总样本数(已证仍 1 秒):
 *   v4.8 铁证: 一次性喂全量 PCM, d 指针不变 n 递增到 cbs, 内容恒 6 字节
 *   → count 语义既非"总样本数"也非"每帧样本数", 编码器本身输出 CNG 静音
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        /* v4.9: 完整 dump QQSilkCodec 方法表 — 找正确 init/编码 API(盲区) */
        {
            unsigned cmc = 0;
            Method *cml = class_copyMethodList(c, &cmc);
            if (cml) {
                TTLog(@"[silk] QQSilkCodec 方法共 %u 个:", cmc);
                for (unsigned j = 0; j < cmc; j++) {
                    TTLog(@"[silk]   %s [%s]", sel_getName(method_getName(cml[j])),
                          method_getTypeEncoding(cml[j]) ?: "?");
                }
                free(cml);
            }
        }
        ...
        /* v4.6: 运行时 dump encode 方法真实签名 — 判 block 参数型 */
        SEL encSel = NSSelectorFromString(@"encode:withSamplesCount:callback:");
        {
            Method m = class_getInstanceMethod(c, encSel);
            if (m) {
                const char *rt = method_copyReturnType(m);
                unsigned int nargs = method_getNumberOfArguments(m);
                NSMutableString *ts = [NSMutableString stringWithCString:(rt?:"?") encoding:NSASCIIStringEncoding];
                [ts appendString:@" ("];
                for (unsigned int ai = 2; ai < nargs; ai++) {
                    char *at = method_copyArgumentType(m, ai);
                    [ts appendFormat:@"%s%s", (ai>2?" ":""), (at?at:"?")];
                    if (at) free(at);
                }
                [ts appendString:@")"];
                TTLog(@"[silk] encode 方法签名: %@", ts);
                if (rt) free(rt);
            }
        }
        if (![codec respondsToSelector:encSel]) { TTLog(@"[silk] encode sel MISS"); return nil; }
        const int16_t *samples = (const int16_t *)pcm.bytes;
        NSUInteger total = pcm.length / 2;           /* 总样本数(16bit mono) */
        /* ⚠️ v4.8 根因判决: encode:withSamplesCount: 的 count 参数语义 = 总样本数,
         * 必须一次性喂入全部 PCM, 让 QQ 编码器自己内部分帧!
         * v4.7 把 PCM 切成 20ms(320样本)×107 帧逐帧 encode → 每帧孤立样本只能
         * 产出 2 字节 CNG 静音帧头(cbs=214=107×2, nMax=214, dump 从 n=6 起恒
         * 29f752f6a121 重复) → 总输出 214B≈0.8kbps 静音, 非 15kbps 语音。
         * 真实录音 3824B/2s => 每 20ms 帧≈38B. 修正: 一次 encode(count=total)。 */
        __block NSMutableData *body = [NSMutableData data];   /* 末次全量快照 = 完整 silk 比特流 */
        __block NSMutableData *concat = [NSMutableData data]; /* 所有 callback d 内容合并(判每帧独立) */
        __block NSUInteger cbs = 0;
        __block uint64_t nMax = 0, nFirst = 0;
        __block NSMutableString *dumpHead = [NSMutableString string];
        /* 一次性喂入全部样本: data = 完整 PCM, count = 总样本数 */
        NSData *allSamples = [NSData dataWithBytes:samples length:total*2];
        ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
            (codec, encSel, allSamples, total, ^(const void *d, uint64_t n) {
                if (!d || !n) return;
                if (cbs == 0) nFirst = n;
                if (cbs < 12) {
                    unsigned char h[12] = {0};
                    memcpy(h, d, n < 12 ? (size_t)n : 12);
                    [dumpHead appendFormat:@"[%llu@%p=", (unsigned long long)n, d];
                    for (int i = 0; i < 12 && i < (int)n; i++)
                        [dumpHead appendFormat:@"%02x", h[i]];
                    [dumpHead appendString:@"]"];
                }
                if (n > nMax) nMax = n;
                [body setData:[NSData dataWithBytes:d length:(NSUInteger)n]];
                [concat appendBytes:d length:(NSUInteger)n];
                cbs++;
            });
        if (cbs == 0) { TTLog(@"[silk] 无帧输出"); return nil; }
        TTLog(@"[silk] cbs=%lu nFirst=%llu nMax=%llu body=%luB concat=%luB dump=%@",
              (unsigned long)cbs, (unsigned long long)nFirst, (unsigned long long)nMax,
              (unsigned long)body.length, (unsigned long)concat.length, dumpHead);
        if (body.length != (NSUInteger)nMax)
            TTLog(@"[silk] ⚠️ body(%lu)≠nMax(%llu) — 末次快照异常, 看 dump",
                  (unsigned long)body.length, (unsigned long long)nMax);
        /* v4.9: concat 若接近真实 silk 量级(>1000B 且 < PCM/4) → 每帧独立拼接才对;
         * 否则 concat 是进度快照重复累计(垃圾), 仍用 body 末次快照 */
        NSData *output;
        if (concat.length > 1000 && concat.length < total*2/4) {
            TTLog(@"[silk] 选 concat(每帧独立拼接) %luB", (unsigned long)concat.length);
            output = concat;
        } else {
            TTLog(@"[silk] 选 body(末次快照) %luB", (unsigned long)body.length);
            output = body;
        }
        NSMutableData *file = [NSMutableData data];
        /* 文件头: 真实模板头优先(0x02开头→11B / '#'→10B), 无模板默认 #!SILK_V3 */
        if (g_realHeadLen >= 10) {
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.9 init (silk 方法表全 dump + callback 取证增强 + 找正确 encode API)");
