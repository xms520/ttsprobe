/*
 * QQFloat_v43.m — v4.3: v4.2 日志判决（QQFloat_2.log 27318B）:
 *   ✅ 不再崩溃! isa 保留 + small-empty 兜底生效 — 全链安全走完, QQ 未闪退
 *   ✅ 内部桥地址算术正确: slide=0x2b14000 → bridge=0x112724158
 *   ❌ 内部桥返回位型垃圾: q0=0x16eecabf0 q1=0x19ba29c52 (非 0xE0/0xD0/0x90 形态)
 *      → 桥址对但单参 ABI 不对: sendAiVoice 调桥时 x1 仍是 _cmd(sel指针),
 *        x2=groupCode, x3=voiceType 全是调用点遗留 — 桥可能消费 x1+
 *      → 本轮深挖: 0x10fc10158 与 0x10fc10154(双参 String retain) 相邻 GOT 槽,
 *        真身无法静态确定(GOT 槽 chained fixup 读不出)
 *
 * v4.3 三处改动:
 *   1.【H 钩子新增】sendPttMsgWithAudioModel: 五参全 hook —
 *      QQ 真实录音发送走的是它(不是 D 的 sendAudioPlacehodlerMsg)!
 *      v3.7 装的 D 钩子从未触发过 = 日志从无 [qq-real] 行的铁证。
 *      用户真实录一条语音 → [qq-real] raw16 + model[0x50] + phInfo raw30 全 dump
 *      → String 真实位型唯一可靠来源 → 下版照抄硬编码
 *   2.【J 钩子新增】sendAiVoiceMsgWithGroupCode: hook —
 *      QQ 自带 AI 语音(反汇编实锤: voiceType 必须==1 才进主流程, 0x6f76="voiceType" 错误串)
 *      用户在群里用 AI 语音时抓 groupCode/timbre/voiceType 全参数
 *   3. 内部桥保留(位型可疑时不用) + small-empty 兜底不变 — 零崩溃路径
 *
 * v4.3 判决树:
 *   - 用户真实录音发送 → [qq-real] sendPtt model=... + raw16 + model[0x50] + phInfo raw30
 *     → 位型照抄进 v4.4 硬编码 → TTS 全链闭环
 *   - 群里用 QQ AI 语音 → [qq-real] sendAiVoice groupCode=... → 备用发送通道
 *   - TTS 合成: 桥位型垃圾→占位不崩(现状不变)
 *
 * ====================== 历史判决存档 ======================
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @try {
                ...
                if (!audioModel) { TTLog(@"[qq] class_createInstance 返回 nil"); }
                     ...
                     * v4.2 修复①(DeepSeek 判决): 整块 memset 会把 [offset 0] 的 isa 清成 0!
                     * 后续任何 objc retain/消息 → [nil+...] 崩。
                     * 改从 offset 8 开始清零(instanceStart 内含 isa 8B), isa 保留 */
                    {
                        size_t isz = class_getInstanceSize(mCls);
                        memset((char *)(__bridge void *)audioModel + 8, 0, isz > 8 ? isz - 8 : 0);
                    }
                    ...
                    /* ---- P2: QQ 内部桥 (v4.2 判决性重写 — DeepSeek 建议采纳) ----
                     * v4.0/v4.1 实锤: dlsym 全路径死路 (two-level 盲区, 带_ 也 MISS)
                     * 本轮静态反汇编新实锤: QQ 主二进制内部就有现成单参桥!
                     *   sendAiVoiceMsgWithGroupCode: (imp 0x1085a2df0) 内:
                     *     0x1085a2e44: mov x0,x4(NSString); bl 0x10fc10158
                     *     → 返回 (x0,x1) = Swift String 16B (后续 0x10fc0ebb4 release q1 侧证实)
                     *   0x10fc10158 = QQ bind 的 NSString→String 单参桥 thunk
                     *   — 不需要 String.Type, 不需要 w3=1, 单参直调!
                     * 调用方式: dyld 枚举 image 找 QQ 主二进制 slide,
                     *   fn = base + (0x10fc10158 - 0x100000000)
                     * ⚠️ 偏移仅对 QQ 9.3.60 (617MB 脱壳二进制) 静态实锤, 换版本需重dump */
                    {
                        typedef struct { uint64_t a; uint64_t b; } BridgeRet;
                        void *qqBridge = NULL;
                        {
                            /* 找 QQ 主 image 的加载 slide */
                            uint32_t ni = _dyld_image_count();
                            for (uint32_t i = 0; i < ni; i++) {
                                const char *nm = _dyld_get_image_name(i);
                                if (!nm || !strstr(nm, "/QQ.app/QQ")) continue;
                                intptr_t slide = _dyld_get_image_vmaddr_slide(i);
                                qqBridge = (void *)(0x10fc10158 + slide);
                                TTLog(@"[qq] qqbase slide=%#lx → bridge=%p",
                                      (unsigned long)slide, qqBridge);
                                break;
                            }
                            if (!qqBridge) TTLog(@"[qq] QQ 主 image 未找到 — 桥跳过");
                        }
                        if (qqBridge) {
                            @try {
                                BridgeRet (*br)(id) = (BridgeRet (*)(id))qqBridge;
                                BridgeRet r = br(silkPath);
                                sq0 = r.a; sq1 = r.b;
                                TTLog(@"[qq] 内部桥 q0=%#llx q1=%#llx",
                             ...
                             * small: q0 高字节 0xE0 系; bridged: q1 高 8 位堆指针 0xD0/0x90 */
                            BOOL plausible = (sq1 >> 56) != 0 || ((sq0 >> 56) & 0xFF) >= 0xE0;
                            if (!plausible) {
                                /* 位型垃圾 → 内部桥 ABI 也有出入 → 试四参形态
                                 * (x0=String.Type 未知, 先跳过 — 单参是主路) */
                                TTLog(@"[qq] 内部桥位型可疑 — 不采用");
                                sq0 = 0; sq1 = 0;
                            } else {
                                stringOK = YES;
                            }
                    ...
                    if (stringOK) {
                        ...
                        /* v4.2 修复②(DeepSeek 判决): (0,0) 不是合法 Swift String!
                         * q0=0/q1=0 时 sendPtt 深层把 q1 当 object 解引用 → addr=0x18 崩 ×6
                         * 借 DeepSeek 建议的最小合法位型: q0=0xE000000000000000
                         *   (count=0 + small-string discriminator 0xE0, q1=0 内联)
                         * 0xE0 高位 = immortal small-string 空串形态, release/retain 均安全。
                         * 仍不真正发送(空 path 发不出), 但不崩 — 拿 [qq-real] 位型后再走真路径 */
                        sq0 = 0xE000000000000000ULL; sq1 = 0;
                        TTLog(@"[qq] ⚠️ 桥不可用 — small-empty 占位 q0=%#llx q1=0 (不真发,不崩)",
                              (unsigned long long)sq0);
                        const char *base = (const char *)(__bridge void *)audioModel;
                        memcpy((void *)(base + 0x10), &sq0, 8);
                        memcpy((void *)(base + 0x18), &sq1, 8);
                        dispatch_async(dispatch_get_main_queue(), ^{
                            self.send.enabled = YES; [self.spinner stopAnimating];
                            self.statusLabel.text = @"桥不可用—先真实录条语音看[qq-real]日志";
                        });
                        return;
                    }
            ...
            id placeholderInfo = nil;
            {
                ...
                if (pCls) {
                    @try {
                        ...
                        if (placeholderInfo) {
                            /* v4.2 修复①同步: 保留 isa, 从 offset 8 起清零 */
                            size_t psz = class_getInstanceSize(pCls);
                            memset((char *)(__bridge void *)placeholderInfo + 8, 0, psz > 8 ? psz - 8 : 0);
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.3 init (H钩子sendPtt真录音抓位型 + J钩子AI语音 + 内部桥)");
