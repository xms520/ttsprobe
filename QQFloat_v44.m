...
- (void)sendDirect {
    ...
    void (^onAudio)(NSData *, NSError *) = ^(NSData *audio, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            TTLog(@"[qq] silk %luB 编码完成 — 进入捕获重放链", (unsigned long)silkData.length);

            /* ===== v4.4: 捕获重放 — 不构造任何对象, 直接复用真实录音模板 =====
             * v4.3 铁证: String 大串 q0=0xC000..|count / q1=0b01|ptr; audioType=1;
             *   duration float@+0x20; phInfo size=24 elementID=(0,0)空串(QQ 自己就这么用)
             * 内部桥 br(silkPath) 崩点判决 → 桥/构造路线全弃
             * 前提: 用户已在 QQ 里真实发过一条语音(H 钩子已捕获 model/phInfo/attrs/path)
             * 步骤: 1) TTS silk 覆写真实 silk 文件路径  2) 更新模板 duration
             *       3) sendPttMsgWithAudioModel:(模板, 模板phInfo, 模板attrs, 自有blocks) */
            ...
            TTLog(@"[qq] silk %luB 覆写真实路径 %@", (unsigned long)silkData.length, realPath);
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.4 init (捕获重放: H钩子抓真实模板+路径解码, 桥/构造全弃)");
