/*
 * QQFloat_v416.m — v4.16: v4.15 对照实验判决落地（QQFloat_8.log 40792B）:
 *   ✅ C1/C2/C3(setEncodeParam+采样率+码率) → cbs=20 有输出(安全组合定案)
 *   ❌ C4(+setSilkSamplesPerFrame) → cbs=0 编码器死 → 禁用!
 *   ❌ C5(+allocEncodeBuffer/initAgc/initNsx) → QQ 直接崩溃重启 → 禁用!
 *      (这就是"卡在合成中"的元凶: 崩溃后发送 block 永不回调, UI 卡死)
 *   注: C1/C2/C3 前10帧 out=40B 是 TTS 音频开头静音段(head32B全零铁证),
 *       全量编码后 voiceFrames 计数才能判真静音/真语音
 *
 * v4.16 改动:
 *   1. 安全组合 C3 固化: setEncodeParam + setSilkSampleRate + setSilkBitRate(24000)
 *      绝不再调 setSilkSamplesPerFrame / allocEncodeBuffer / initAgc / initNsx
 *   2. 全量分帧编码(320样本/帧), 每帧增量 = [u16LE len][frame] 打包
 *   3. voiceFrames(>4B帧)计数 — 判静音/语音 + 总码率日志
 *   4. 发送 UI 15s 超时兜底 — 任何异常不再永久卡"合成中"
 *
 * v4.16 判决树:
 *   - [silk] C3全量 voiceFrames>0 → 真实语音帧 → 播放正常=终审✅
 *   - voiceFrames=0 → 全静音帧 → VAD 判静音 → v4.17 查 PCM 增益(放大3x)/采样率
 *   - 崩溃应根除(C5 已删) — 若仍崩看 crash log
 *
 * ====================== v4.15 判决存档 ======================
 * QQFloat_v415.m — v4.15: 6组合对照实验 → C4 spf=编码器死, C5 全家桶=崩溃源
 */
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        /* v4.16: v4.15 对照实验判决落地:
         *   C1/C2/C3(含 setSilkSampleRate+setSilkBitRate) → cbs=20 有输出 安全
         *   C4(+setSilkSamplesPerFrame) → cbs=0 编码器死 禁用!
         *   C5(+allocEncodeBuffer/initAgc/initNsx) → QQ 直接崩溃 禁用! (卡合成中元凶)
         * 安全组合 = setEncodeParam + setSilkSampleRate + setSilkBitRate (C3)
         * 全量分帧编码(320样本/帧), 每帧增量=[u16 len][frame] 打包 */
        ...
        /* ⚠️ 绝不调用: setSilkSamplesPerFrame(cbs=0) / allocEncodeBuffer / initAgc / initNsx(崩溃) */

        const int16_t *samples = (const int16_t *)pcm.bytes;
        NSUInteger total = pcm.length / 2;
        NSUInteger frame = 20 * rate / 1000;   /* 320 样本/帧 */
        ...
        NSUInteger nF = 0;
        NSUInteger voiceFrames = 0;   /* 非静音帧计数(帧长>4B) */
        for (NSUInteger off = 0; off + frame <= total; off += frame) {
            @autoreleasepool {
                ...
                if (lastN > before && lastD) {
                    ...
                    if (flen > 4) voiceFrames++;
                    nF++;
                }
            }
        }
        if (nF == 0 || packed.length == 0) {
            TTLog(@"[silk] 全量编码无输出 (cbs=%lu total=%lu)", (unsigned long)cbs, (unsigned long)total);
            return nil;
        }
        double kbps = (double)packed.length * 8 / ((double)total / rate) / 1000.0;
        TTLog(@"[silk] C3全量: cbs=%lu frames=%lu voiceFrames=%lu out=%luB (%.1f kbps)",
              (unsigned long)cbs, (unsigned long)nF, (unsigned long)voiceFrames,
              (unsigned long)packed.length, kbps);
        if (voiceFrames == 0)
            TTLog(@"[silk] ⚠️ 全部帧≤4B(纯静音) — VAD 判静音, 查 PCM 增益/采样率");
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.16 init (安全组合C3固化 + 删除崩溃源 + voiceFrames判决)");
