...
static NSString *TTSSendVoice(NSData *pcmData, NSString *toUsr) {
    ...
    TTLog(@"[send] PCM fed=%lu bytes chunks=%lu", (unsigned long)fed, (unsigned long)seq);
     ...
     */
    TTLog(@"[send] prepareSend accepted; NO upload-success claim. Watch subsequent callbacks/logs.");
    return nil;
}
