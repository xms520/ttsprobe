 ...
 * 找不到按钮则退回手动提示(注入仍有效)。 */
static BOOL g_autoPttEnabled = YES;
...
- (void)togglePanel {
    ...
    self.send.frame = CGRectMake(12, 181, 182, 40);   /* v4.20: 右侧留给「🎙 注入」 */
    ...
    /* v4.20: 🎙 注入 —— 用我们的 PCM 顶替 QQ 录音数据, 走 QQ 官方编码/发送 */
    UIButton *injB = [UIButton buttonWithType:UIButtonTypeSystem];
    injB.frame = CGRectMake(200, 181, 88, 40);
    injB.backgroundColor = [UIColor colorWithRed:.95 green:.55 blue:.15 alpha:1];
    injB.layer.cornerRadius = 9;
    [injB setTitle:@"🎙 注入" forState:UIControlStateNormal];
    [injB setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
    injB.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    [injB addTarget:self action:@selector(injectTapped) forControlEvents:UIControlEventTouchUpInside];
    [panel addSubview:injB];
...
- (void)armInjectionWithPcm:(NSData *)pcm tag:(NSString *)tag {
    ...
    self.statusLabel.text = [NSString stringWithFormat:@"✔ 已合成 %.1f 秒 → 回聊天按住说话约 %.1f 秒后松手", sec, sec];
...
static void QQFloatV2Init(void) {
    ...
    InstallCodecHook();
    InstallProbeHooks();
    TTLog(@"QQFloat v4.23 init (auto-ptt 遍历全部窗口 + UIControl 驱动 + 候选日志)");
