...
static UIButton *TTFindVoiceButton(void) {
    NSArray *wins = UIApplication.sharedApplication.windows;
    UIView *root = nil;
    for (UIWindow *w in wins) { if (w.isKeyWindow) { root = w; break; } }
    if (!root && wins.count) root = wins.firstObject;
    if (!root) return nil;
    NSMutableArray *queue = [NSMutableArray arrayWithObject:root];
    NSMutableArray *labels = [NSMutableArray array];
    UIButton *hit = nil;
    NSUInteger guard = 0;
    while (queue.count && guard++ < 6000) {
        ...
        if ([v isKindOfClass:[UIButton class]]) {
            ...
            if (labels.count < 40)
                [labels addObject:[NSString stringWithFormat:@"[%@|%@]", t, a]];
            if (!hit && ([t containsString:@"按住"] || [a containsString:@"按住"] ||
                         [t containsString:@"说话"] || [a containsString:@"说话"])) hit = b;
        }
        for (UIView *sub in v.subviews) [queue addObject:sub];
    }
    if (labels.count)
        TTLog(@"[auto-ptt] 按钮候选(%lu): %@", (unsigned long)labels.count,
...
- (void)autoPttForSeconds:(double)sec {
    g_autoPttTried++;
    UIButton *b = TTFindVoiceButton();
    if (!b) {
        TTLog(@"[auto-ptt] 未找到「按住说话」按钮(可能不在聊天语音模式) — 请手动按住 %.1fs", sec);
        self.statusLabel.text = [NSString stringWithFormat:@"请手动按住说话约 %.1f 秒后松手", sec];
        return;
    }
    TTLog(@"[auto-ptt] 命中 title=「%@」a11y=「%@」→ 自动按住 %.1fs",
          b.currentTitle ?: @"", b.accessibilityLabel ?: @"", sec);
...
static void QQFloatV2Init(void) {
    ...
    InstallCodecHook();
    InstallProbeHooks();
    TTLog(@"QQFloat v4.22 init (合成→注入待命+自动按住说话; 待命 60s 超时保护)");
