...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        /* encode:withSamplesCount:callback: 返回 uint64, 帧数据经 callback 递出 */
        ...
        __block BOOL packedFmt = NO;           /* callback 输出已含 [u16 len][frame]? */
        __block BOOL fmtChecked = NO;
        __block NSUInteger frames = 0, firstN = 0;
        __block NSMutableData *firstHeadData = [NSMutableData data];  /* 首帧头取证(__block 不支持C数组) */
        for (NSUInteger off = 0; off + frame <= total; off += frame) {
            ...
            ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
                (codec, encSel, chunk, frame, ^(const void *d, uint64_t n) {
                    if (!d || !n) return;
                    if (!fmtChecked && n >= 4) {
                        /* 首帧判定: 前导 u16(LE) == n-2 → 已是 [len][frame] 打包格式 */
                        uint16_t l0 = *(const uint16_t *)d;
                        packedFmt = (l0 == (uint16_t)(n - 2));
                        fmtChecked = YES; firstN = (NSUInteger)n;
                        [firstHeadData setData:[NSData dataWithBytes:d
                                                               length:(NSUInteger)(n < 16 ? n : 16)]];
                    }
                    if (packedFmt) {
        ...
        const unsigned char *fh = (const unsigned char *)firstHeadData.bytes;
        TTLog(@"[silk] frames=%lu fmt=%@ 首帧%luB头=%02x%02x%02x%02x%02x%02x%02x%02x",
              (unsigned long)frames, packedFmt ? @"packed" : @"raw(已补len前缀)",
              (unsigned long)firstN,
              fh[0],fh[1],fh[2],fh[3],fh[4],fh[5],fh[6],fh[7]);
        NSMutableData *file = [NSMutableData data];
        /* 文件头: 真实模板头优先(10B/11B magic 照抄), 无模板默认 "#!SILK_V3" */
        if (g_realHeadLen >= 10) {
            NSUInteger hlen = g_realHeadLen < 16 ? g_realHeadLen : 16;  /* magic 区(首帧数据前) */
            /* QQ silk 变体: "\x02#!SILK_V3"(11B) / "#!SILK_V3"(10B) — 头 len 以真实文件为准:
             * 0x02 开头 → 11B; '#' 开头 → 10B */
            if (g_realHead[0] == 0x02) hlen = 11;
            else hlen = 10;
            [file appendBytes:g_realHead length:hlen];
            TTLog(@"[silk] 文件头=真实模板 %luB (%@)", (unsigned long)hlen,
                  [[NSString alloc] initWithBytes:g_realHead length:hlen encoding:NSUTF8StringEncoding]);
        } else {
...
- (void)sendDirect {
    ...
    void (^onAudio)(NSData *, NSError *) = ^(NSData *audio, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            TTLog(@"[qq] silk %luB 编码完成(fmt自适应+真实头) — 进入覆写", (unsigned long)silkData.length);
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.5 init (24kHz silk + 真实文件头照抄 + 帧格式自适应 + 文件头取证)");
