/*
 * QQFloat_v415.m — v4.15: v4.14 判决（QQFloat_7.log）→ 分帧也 cbs=0, 对照实验定凶手:
 *   ❌ v4.14 分帧(320)+全参数 → cbs=0 | v4.8 分帧+只setEncodeParam → cbs=214 有输出
 *   → v4.14 新加的某个 init 调用(allocEncodeBuffer/setSilkSampleRate等)破坏了编码器!
 *   ✅ 另发现帧格式错误: QQ silk 文件 = [u16LE 帧长][帧数据] 逐帧打包(真实头[11:13]=帧长)
 *      我们一直裸帧流直拼 → 播放器错位 → 只播1秒 的第二原因!
 *
 * v4.15 改动:
 *   1. 6 组合对照实验(各编前10帧): C1=只setEncodeParam(v4.8对照) C2=+码率 C3=+采样率+码率
 *      C4=采样率+帧样本+码率 C5=全家桶(v4.14对照) C6=采样率+码率(无setEncodeParam)
 *      → 一次真机测试定位: 谁破坏编码器 + 谁能输出真实语音帧(10帧静音≈22B/语音≈200B+)
 *   2. 帧提取修正: 每帧增量 = d[before..after](帧边界=encode调用间n增量)
 *   3. 文件格式修正: 10B magic(02+"#!SILK_V3") + [u16LE len][frame] 逐帧打包
 *
 * v4.15 判决树:
 *   - C1 cbs=0 → 环境变了(v4.8 老代码在当前 QQ 也失效) → 看其他组合
 *   - C2/C3/C6 out>100B → 码率参数生效, 真实语音帧 → 全量编码 → 终审✅
 *   - 全部 out≈22B 静音 → 编码器VAD判静音 → v4.16 检查 PCM 字节序/增益
 *   - C5 cbs=0 且 C4 有输出 → allocEncodeBuffer/initAgc/initNsx 是凶手
 *
 * ====================== v4.14 判决存档 ======================
 * QQFloat_v414.m — v4.14: 分帧+全参数 → cbs=0(init链破坏编码器)
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        /* v4.15: 对照实验 — v4.14 分帧也 cbs=0, 对比 v4.8(分帧+只setEncodeParam=有callback)
         * → 新加的某个 init 调用破坏了编码器!
         * 6 组合各编前10帧找真相 + 帧提取修正(每帧增量=d[before..after]) +
         * 文件格式修正(QQ silk=[u16LE len][frame]逐帧打包, 之前裸流直拼是错的) */
        ...
        if (![c instancesRespondToSelector:cbSel]) { TTLog(@"[silk] callback sel MISS"); return nil; }

        const int16_t *samples = (const int16_t *)pcm.bytes;
        NSUInteger total = pcm.length / 2;
        NSUInteger frame = 20 * rate / 1000;   /* 320 */

        /* 单组合测试: 返回 [u16LE len][frame]... 打包数据; outCbs/outBytes 取证 */
        NSData *(^tryCombo)(const char*, BOOL, BOOL, BOOL, BOOL, BOOL, BOOL, NSUInteger, NSUInteger*, NSUInteger*) =
        ^(const char *name, BOOL setRate, BOOL setSpf, BOOL setBr, BOOL setBuf,
          BOOL setAgc, BOOL setNsx, NSUInteger maxFrames, NSUInteger *outCbs, NSUInteger *outBytes) {
            id codec = [[c alloc] init];
            if ([codec respondsToSelector:epSel]) ((void (*)(id, SEL))objc_msgSend)(codec, epSel);
            if (setRate && [codec respondsToSelector:srSel]) ((void (*)(id, SEL, uint64_t))objc_msgSend)(codec, srSel, (uint64_t)rate);
            if (setSpf && [codec respondsToSelector:spfSel]) ((void (*)(id, SEL, uint64_t))objc_msgSend)(codec, spfSel, (uint64_t)(rate/50));
            if (setBr  && [codec respondsToSelector:brSel]) ((void (*)(id, SEL, uint64_t))objc_msgSend)(codec, brSel, (uint64_t)24000);
            if (setBuf && [codec respondsToSelector:aebSel]) ((BOOL (*)(id, SEL))objc_msgSend)(codec, aebSel);
            if (setAgc && [codec respondsToSelector:iagcSel])((BOOL (*)(id, SEL))objc_msgSend)(codec, iagcSel);
            if (setNsx && [codec respondsToSelector:insxSel])((BOOL (*)(id, SEL))objc_msgSend)(codec, insxSel);
            __block NSUInteger cbs2 = 0;
            __block uint64_t lastN = 0;
            __block const void *lastD = NULL;
            NSMutableData *packed = [NSMutableData data];   /* [u16 len][frame]... */
            NSUInteger nF = 0;
                ...
                @autoreleasepool {
                    ...
                    ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
                        (codec, cbSel, chunk, (NSUInteger)frame, ^(const void *d, uint64_t n) {
                            if (!d || !n) return;
                            lastD = d; lastN = n; cbs2++;
                        });
                    if (lastN > before && lastD) {   /* 本帧增量 = d[before..lastN] */
                        uint16_t flen = (uint16_t)(lastN - before);
                        [packed appendBytes:&flen length:2];
                        [packed appendBytes:((const unsigned char*)lastD + before) length:flen];
                        nF++;
                    }
                }
            }
            *outCbs = cbs2; *outBytes = packed.length;
            TTLog(@"[silk] %s: cbs=%lu frames=%lu out=%luB", name,
                  (unsigned long)cbs2, (unsigned long)nF, (unsigned long)packed.length);
            return packed;
        };

        /* 6 组合各编前 10 帧(对照实验) */
        NSUInteger cbsV = 0, outV = 0;
        NSData *r1 = tryCombo("C1-只setEncodeParam(v4.8对照)", NO,NO,NO,NO,NO,NO, 10, &cbsV, &outV);
        NSData *r2 = tryCombo("C2-+码率24000",               NO,NO,YES,NO,NO,NO, 10, &cbsV, &outV);
        NSData *r3 = tryCombo("C3-+采样率+码率",              YES,NO,YES,NO,NO,NO, 10, &cbsV, &outV);
        NSData *r4 = tryCombo("C4-采样率+帧样本+码率",         YES,YES,YES,NO,NO,NO, 10, &cbsV, &outV);
        NSData *r5 = tryCombo("C5-全家桶(v4.14对照)",         YES,YES,YES,YES,YES,YES, 10, &cbsV, &outV);
        NSData *r6 = tryCombo("C6-采样率+码率(无setEncodeParam)", YES,NO,YES,NO,NO,NO, 10, &cbsV, &outV);

        /* 选输出最大的组合(10帧静音≈22B; 真实语音≈200B+) */
        NSArray *results = @[r1 ?: [NSData data], r2 ?: [NSData data], r3 ?: [NSData data],
                             r4 ?: [NSData data], r5 ?: [NSData data], r6 ?: [NSData data]];
        __block NSData *best = results[0]; __block NSUInteger bi = 0;
        [results enumerateObjectsUsingBlock:^(NSData *d, NSUInteger i, BOOL *stop) {
            if (d.length > best.length) { best = d; bi = i; }
        }];
        TTLog(@"[silk] 胜出组合: C%lu (%luB/10帧) — 全量编码", (unsigned long)bi+1, (unsigned long)best.length);

        /* 用胜出组合全量编码 */
        NSData *full = tryCombo([[NSString stringWithFormat:@"C%lu-full", (unsigned long)bi+1] UTF8String],
                                bi >= 2, bi == 3 || bi == 4, YES, bi == 4, bi == 4, bi == 4,
                                NSUIntegerMax, &cbsV, &outV);
        if (!full || full.length == 0) {
            TTLog(@"[silk] 全量编码失败 — 用 C1 兜底");
            full = tryCombo("C1-full-兜底", NO,NO,NO,NO,NO,NO, NSUIntegerMax, &cbsV, &outV);
        }
        if (!full || full.length == 0) { TTLog(@"[silk] 编码彻底失败"); return nil; }

        /* 文件 = 10B magic(0x02+"#!SILK_V3") + [u16LE len][frame]... */
        NSMutableData *file = [NSMutableData data];
        if (g_realHeadLen >= 10) {
            [file appendBytes:g_realHead length:10];   /* 10B magic: 02 23 21 53 49 4c 4b 5f 56 33 */
            TTLog(@"[silk] 文件头=真实模板 magic 10B");
        } else {
            [file appendBytes:"#!SILK_V3" length:9];
            TTLog(@"[silk] 文件头=默认 #!SILK_V3");
        }
        [file appendData:full];
        TTLog(@"[silk] silk文件共 %luB (帧打包格式)", (unsigned long)file.length);
        return file;
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.15 init (6组合对照实验 + 帧格式[u16len]修正)");
