/*
 * QQFloat_v417.m — v4.17: v4.16 判决（QQFloat_9.log 4757B）→ VAD 判静音三疑点:
 *   ✅ 零崩溃零卡死! 发送链全通(code=0), 帧数完美对齐(2495ms→124帧)
 *   ❌ 124 帧全 2B CNG 静音帧(1.6kbps) — 编码器把 PCM 判成静音
 *   疑点A: C1=C2=C3 输出全同(40B) → setSilkSampleRate/BitRate 在 setEncodeParam
 *          之后调用 = no-op! 编码器初始化后 setter 不再生效
 *          → B 路: 参数在 setEncodeParam 之前设置(C6 补测, v4.15 因 C5 崩溃没跑到)
 *   疑点B: maxAbs=27228 只是全文件峰值, 不证明整体有语音 → dump 中段+RMS+占比
 *   疑点C: VAD 阈值/DTX 锁死 → C/D 路: 增益 4x + 跳过开头静音
 *
 * v4.17 改动:
 *   1. [pcm] 诊断行: mid32B + rms + peak + |v|>100/1000 占比
 *   2. 4 路各编 20 帧对照: A=参数后设(对照) B=参数先设 C=先设+增益4x D=后设+增益4x
 *   3. 胜出路全量编码
 *
 * v4.17 判决树:
 *   - [pcm] |v|>1000 占比 <5% → PCM 本身就是静音(解码链 bug) → 修 DecodeToPCM
 *   - B 路 voice>0 → setter 顺序是关键 → 参数先设 = 修复!
 *   - C/D 路 voice>0 → 增益修复 → 全量即出语音
 *   - 全部 voice=0 且 PCM 占比正常 → VAD 参数在更深处 → 换 QQSilkRecorder 管线
 *
 * ====================== v4.16 判决存档 ======================
 * QQFloat_v416.m — v4.16: 安全组合C3+124帧全CNG(VAD判静音) — 崩溃/卡死已根除
 */
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        /* v4.17: v4.16 判决 — C3 全量 124 帧全 2B CNG(VAD 判静音) 但零崩溃发送通!
         * 三个疑点一次判死:
         *   A. setSilkSampleRate/BitRate 在 setEncodeParam 后调用=no-op(C1=C2=C3 输出全同铁证)
         *      → D 组: 参数在 setEncodeParam **之前**设置(C6 补测, v4.15 因 C5 崩溃没跑到)
         *   B. maxAbs=峰值不能证明整体有语音 → dump 中段数据+有效样本占比+RMS
         *   C. VAD 阈值/DTX 锁死 → 增益 4x + 跳过开头静音对比 */
        const int16_t *samples = (const int16_t *)pcm.bytes;
        NSUInteger total = pcm.length / 2;

        /* ===== 诊断1: PCM 真实形态 ===== */
        {
            NSUInteger nz = 0, above = 0; long long sumSq = 0;
            int16_t peak = 0;
            ...
            /* 中段 32B hex */
            NSUInteger mid = total / 2;
            if (mid + 16 > total) mid = total > 16 ? total - 16 : 0;
            const unsigned char *mp = (const unsigned char *)(samples + mid);
            NSMutableString *hx = [NSMutableString string];
            for (NSUInteger k = 0; k < 32; k++) [hx appendFormat:@"%02x", mp[k]];
            TTLog(@"[pcm] mid32B=%@ rms=%.0f peak=%d |v|>100:%lu%% |v|>1000:%lu%%",
                  hx, rms, peak, (unsigned long)(nz*100/total), (unsigned long)(above*100/total));
        }
        ...
        /* 单次实验: gain=增益倍数, paramFirst=参数在setEncodeParam前设, maxF=帧上限 */
        NSData *(^tryPath)(const char*, float, BOOL, NSUInteger) =
        ^(const char *name, float gain, BOOL paramFirst, NSUInteger maxF) {
            ...
            /* 增益 + 跳过开头静音(第一个 |v|>500 的样本) */
            NSUInteger begin = 0;
            if (gain > 1.0f) {
                while (begin < total) {
                    int16_t v = samples[begin];
                    if ((v < 0 ? -v : v) > 500) break;
                    begin++;
                }
            }
            int16_t *buf = NULL;
            const int16_t *inp = samples;
            NSUInteger avail = total;
            if (gain > 1.0f && begin < total) {
                avail = total - begin;
                buf = (int16_t *)malloc(avail * 2);
                for (NSUInteger i = 0; i < avail; i++) {
                    int s32 = (int)((float)samples[begin + i] * gain);
            ...
            NSMutableData *packed = [NSMutableData data];
            NSUInteger nF = 0, voiceF = 0;
            for (NSUInteger off = 0; off + frame <= avail && nF < maxF; off += frame) {
                @autoreleasepool {
                    NSData *chunk = [NSData dataWithBytes:inp+off length:frame*2];
            ...
            if (buf) free(buf);
            TTLog(@"[silk] %s: cbs=%lu frames=%lu voice=%lu out=%luB", name,
                  (unsigned long)cbs2, (unsigned long)nF, (unsigned long)voiceF,
                  (unsigned long)packed.length);
            return packed;
        };

        /* 4 路各编 20 帧 */
        NSData *dA = tryPath("A-参数后设(v4.16对照)", 1.0f, NO, 20);
        NSData *dB = tryPath("B-参数先设(setEncodeParam前)", 1.0f, YES, 20);
        NSData *dC = tryPath("C-参数先设+增益4x+跳静音", 4.0f, YES, 20);
        NSData *dD = tryPath("D-参数后设+增益4x+跳静音", 4.0f, NO, 20);

        /* 选 voice 帧多的路全量编码(voice=0 时选输出最大的) */
        NSArray *rs = @[dA ?: [NSData data], dB ?: [NSData data], dC ?: [NSData data], dD ?: [NSData data]];
        __block NSUInteger bi = 0; __block NSUInteger bestScore = 0;
        [rs enumerateObjectsUsingBlock:^(NSData *d, NSUInteger i, BOOL *st) {
            if (d.length > bestScore) { bestScore = d.length; bi = i; }
        }];
        const char *names[] = {"A", "B", "C", "D"};
        float gains[] = {1.0f, 1.0f, 4.0f, 4.0f};
        BOOL firsts[] = {NO, YES, YES, NO};
        TTLog(@"[silk] 胜出: %s — 全量编码", names[bi]);
        NSData *full = tryPath("FULL", gains[bi], firsts[bi], NSUIntegerMax);
        if (!full || full.length == 0) { TTLog(@"[silk] 编码失败"); return nil; }
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.17 init (PCM诊断 + setter顺序对照 + 增益4x三路判决)");
