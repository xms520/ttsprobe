        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @try {
                ...
                if (!audioModel) { TTLog(@"[qq] class_createInstance 返回 nil"); }
                     ...
                     *   P2 dlsym 双参桥(v3.7 路径, 补回第三级符号) + 位型判定
                    ...
                    /* ---- P1: 借 QQ 官方 setter 产真 Swift String ---- */
                    Class sendCls = NSClassFromString(@"_TtC18INTAIOChatProtocol20NTSendableAudioModel");
                    TTLog(@"[qq] sendCls=%@", sendCls);
                    if (sendCls) {
                        SEL setSel = NSSelectorFromString(@"setAudioFilePath:");
                        @try {
                            id lend = (id)class_createInstance(sendCls, 0);
                            if (lend && [lend respondsToSelector:setSel]) {
                                /* 先清零(跳过 Swift init 的内存是垃圾), 再走官方 setter */
                                memset((__bridge void *)lend, 0, class_getInstanceSize(sendCls));
                                ((void (*)(id, SEL, id))objc_msgSend)(lend, setSel, silkPath);
                                const char *lb = (const char *)(__bridge void *)lend;
                                uint64_t lq0 = *(const uint64_t *)(lb + 0x10);
                                uint64_t lq1 = *(const uint64_t *)(lb + 0x18);
                                TTLog(@"[qq] P1 setter 产 String q0=%#llx q1=%#llx",
                                      (unsigned long long)lq0, (unsigned long long)lq1);
                                /* 位型判定: q0 低位是 count+flags(q1 高位判 tagged),
                                 * 合法形态: q1 高 8 位非零(bridged object/tagged) 或 q0 高字节=0xE0 系(small) */
                                BOOL plausible = ((lq1 >> 56) != 0) ||
                                                 ((lq0 >> 56) != 0) ||
                                                 (lq0 == 0 && lq1 == 0);
                                if (plausible && !(lq0 == 0 && lq1 == 0)) {
                                    sq0 = lq0; sq1 = lq1; stringOK = YES;
                                    TTLog(@"[qq] P1 成立 — 官方 setter 桥落地");
                                } else {
                                    TTLog(@"[qq] P1 位型不合理, 走 P2");
                                }
                            } else {
                                TTLog(@"[qq] P1 lend/responds MISS");
                            }
                        } @catch (NSException *e) {
                            TTLog(@"[qq] P1 setter 异常 %@", e);
                        }
                    }

                    /* ---- P2: dlsym 双参桥 (v3.7 路径 + 补回第三级符号) ---- */
                    if (!stringOK) {
                        ...
                        if (!bridgeSym) {
                            /* v3.8 补回 v3.6 (bed526e) 命中的第三级符号 — v3.7 手术时误删 */
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v3.8 init (P1 借 setter 产 String + 正序布局 + P2/P3 兜底)");
