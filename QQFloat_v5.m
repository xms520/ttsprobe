/*
 * QQFloat_v418.m — v4.18: v4.17 判决（QQFloat_10.log 41150B）→ 编码器从未进入工作状态:
 *   ✅ PCM 完全正常: rms=2468 peak=11587 |v|>1000 占 46% — 真实语音!
 *   ❌ A/B/C/D 四路全同 voice=0(每帧2B CNG) — setter顺序无关/增益无关/参数无关
 *   → [[QQSilkCodec alloc] init] 实例内部 encoder state 从未初始化, 全 CNG 兜底
 *   QQ 真实录音能出 17B+/帧 → 初始化发生在 QQ 内部路径(QQSilkRecorder.initEncoder)
 *
 * v4.18 三路判决 + 终极取证:
 *   1. swizzle hook encode:withSamplesCount:callback: — 捕获 QQ 真实录音调用的
 *      codec 实例 + count 参数(判 QQ 真实帧大小/采样率!) + data 头 16B
 *   2. REAL 路: 用 hook 捕获的 QQ 真实 codec 实例编码(必然被 QQ 正确初始化)
 *   3. REC 路: QQSilkRecorder → initEncoder → encoder 内部实例(官方初始化路径)
 ...
 * ===== v5.5（第三后端换本地系统语音 + 键盘行为）=====
 * edge-tts 判决：微软已按客户端指纹封锁公开端点（speech.platform.bing.com / api.msedgeservices.com，
 *   多 token 算法/UA/Origin/新老域名组合全部 403；本机 python ssl 与真机 URLSession 双端验证）→ 不可用
 * 替代：AVSpeechSynthesizer.write(iOS13+) 离线合成 → 16k mono WAV → DecodeToPCM → 原发送链
 *   音色=系统已装语音（中文优先, 增强(高质量)排前, 最多16个），显示名 "本地·名字(语言·增强)"
 * 键盘行为（用户要求）：打开音色列表/打赏 → 收起键盘；选中音色 → 收起键盘；点搜索框 → 正常弹键盘
 ...
 *             → fishhook AudioQueueNewInput 的 trampoline 把 TTS PCM 当麦克风数据喂进录音管线
 *             → PCM 喂完 + 0.3s → 主线程复刻 StopRecord → 微信自己的结束链发送（全自动, 无需用户操作）
 *   QQ 宿主 : 原 QQ 链不变（合成 → 注入待命 → 手动按住语音按钮发送）
 * 宿主判断: NSBundle.mainBundle.bundleIdentifier == com.tencent.xin*
 * 微信链依赖: fishhook.c（编译时一起编）
 * ===== v4.24（按用户要求做减法）=====
 * v4.23 日志: [auto-ptt] 命中 QQPttRecordBtn a11y=「按住说话」→ 自动按住 1.7s → 已抬起(发送)
 *   但用户反馈仍无法自动发送 → QQPttRecordBtn 走真实触摸(touchesBegan/Ended), 不响应
 *   sendActionsForControlEvents:(target-action), 故该路封存(g_autoPttEnabled=NO)
 * v4.24: ① 去掉 UI 上的「🎙 注入」按钮, 「合成语音」恢复整行
 *        ② 停用自动按住(代码保留, 开关置 NO)
 *        ③ 流程回归最简: 输入文字 → 点「合成语音」(自动注入待命) → 手动按住语音按钮 N 秒 → 松手 = TTS 语音条
 * ===== v4.23（修 v4.22「找不到按住说话按钮」）=====
 * v4.22 日志: [auto-ptt] 按钮候选(4): [✕|] [打赏|] [1️⃣ 合成语音|] [🎙 注入|]
 *   → 根因: 我们的浮窗(TTSPassWindow, level=Alert+100) 抢了 keyWindow, v4.22 只从 keyWindow 开始遍历
 *     → 只看到自己面板按钮, 完全看不到 QQ 的控件
 * 修复: 遍历 UIApplication.windows 全部窗口(跳过 TTSPassWindow/hidden) +
 *       匹配范围扩大到所有 UIView(title/text/a11y/identifier 含「按住」「说话」) +
 *       命中非 UIControl 时向上找最近 UIControl + 打印候选(class{文本}) 供精确定位
 * ===== v4.22（一键合成语音：注入待命 + 自动「按住说话」）=====
 * v4.21 日志(v4.21 log): [rec-hook] QQSIlkRecorder.initEncoder self=0x282b5aeb0 ✅
 *                        [clean-hook] QQ 调用 cleanEncode self=0x282b445a0 ✅(顺序: initEncoder → cleanEncode → encode)
 *                        [qq] v4.21 合成完成 → 自动注入待命: 35818B @16000Hz ≈ 1.12s ✅
 *                        [inject] 注入数据已用完(23636 样本) — 续填静音 ✅
 * v4.22: ① 合成完成 → 自动「替用户按住说话」: 遍历视图树找「按住说话」按钮 → touchDown → N 秒 → touchUpInside
 *          找不到按钮则退回"请手动按住 N 秒"(注入仍有效, 零崩溃风险: 只碰 UI 控件)
 *        ② 注入待命统一入口 armInjectionWithPcm:tag: + 60 秒未使用自动取消(防误替换真实录音)
 *        ③ 日志 [auto-ptt] 按钮候选列表 → 便于下一版精确锁定按钮
 * ===== v4.21（注入路已验证成功 → 合成按钮改为自动进入注入待命）=====
 * v4.20 实测(v4.20 日志): 注入路 100% 成功(语音条可播放) ✅
 *   但自编码链发出去的是"1 秒静音录音": codec配置 sr=16000 br=16000 spf=320(帧长本来就对!),
 *   R1 REAL原样 avg=2B/帧 voice=1/87, R2 REAL+C3 voice=0, O1 OWN voice=0
 *   → 根因不是帧长/采样率, 而是 QQ 在录音结束后调用 cleanEncode 清空了编码器状态
 *     → 独立编码时全走 CNG 兜底帧
 * v4.21: ①「合成语音」= 合成 + 自动进入注入待命(提示按住说话 N 秒), 旧自编码/覆写/重放链停用
 *        ② 注入数据用完后续填静音(防真实录音尾音混入), ~20s 后自动关
 *        ③ 新增 cleanEncode / QQSIlkRecorder.initEncoder 探针(为"免按住说话"路线铺路)
 * ===== v4.20（修 v4.19 合成语音闪退 + 帧长根因）=====
 * 1) 崩溃根因: v4.18 引入的 REC 路(手动 alloc QQSilkRecorder + initWithSampleRate:andBitRate:)
 *    日志 4 段全部停在 "[silk] REAL(QQ实例): ..." 之后 → REC 段内崩(QQSilkRecorder 手造实例) → 整段删除
 * 2) 关键突破: REAL 路(QQ 真实录音的 codec 实例)首次出非零帧 voice=1/20 —— v4.17 及以前全 0
 * 3) 帧长根因: v4.8~v4.19 一直按 16kHz/320 样本切帧; 若 QQ codec 是 24kHz(480/帧) → 永远喂不满一帧
 *    → 编码器饥饿 → 全 2B CNG 帧 → 这就是所有"全静音"结论的来源
 *    v4.20: 从 codec 读 silkSampleRate/silkBitRate/silkSamplesPerFrame → PCM 重采样 + 按真实帧长切帧
 * 4) 三路全量编码判决: R1=REAL原样 / R2=REAL+C3重设参数(编完恢复) / O1=自建对照 → voice 帧多者胜
 * 5) 新增「🎙 注入」: 用我们的 PCM 顶替 QQ 录音数据 → QQ 官方编码/发送(格式 100% 可播放)
 * ===== v4.19（修 v4.18 按住说话闪退）=====
 * 崩溃根因(真机 QQFloat.log 4 段铁证):
 *   QQSilkCodec.encode:withSamplesCount:callback: type encoding = [Q40@0:8@16Q24@?32]
 *   → 返回类型 Q(输出字节数), 非对象; v4.18 hook 声明 id 返回 + return (id(*)(...))orig(...)
 *   → -fobjc-arc 按 +0 autoreleased 对象处理 → 对整数 x0 做 autorelease → SIGSEGV
 *   → 每次按住说话(QQ 真实录音调用 encode) 立刻闪退
 * 修复: hook 返回 uint64_t + 转发强转 uint64_t(...); 日志改主队列异步; 安装时打印 type encoding
 * codec-hook 捕获到的真实参数(本次日志): count=7168 len=14336 head=全零 / count=8000 len=16000 head=1d01f000...
 *   → count/2 == len 且为 16k 采样整数倍样本 → QQ 逐块喂 PCM(非单帧), 采样率待 REAL/REC 判决
 * v4.18 判决树:
 *   - [codec-hook] count=480 → QQ 用 24kHz! → PCM 转 24kHz 重新编码(v4.19)
 *   - [codec-hook] count=320 → 帧大小对, 问题在 codec 初始化 → REAL/REC voice>0 即修复
 *   - REAL voice>0 → 用 QQ 实例 → 终审✅
 *   - REC voice>0 → Recorder.initEncoder 是关键 → 终审✅
 *   - 全部 voice=0 → encode 深层有隐藏状态 → 用 hook 深挖 QQ 调用序列
 *
 * ====================== v4.17 判决存档 ======================
 * QQFloat_v417.m — v4.17: PCM正常但四路全 voice=0 → 编码器初始化是关键
 */
...
/* ==================== 日志 ==================== */
/* ===== v5.0 双宿主：微信链接口（wx_chain.m 同编的另一个编译单元） =====
 * 微信宿主 → 面板「合成语音」调 WXChainSendWithPcm() → 微信自己录音/编码/发送（全自动）
 * QQ 宿主  → 原 QQ 链（注入待命 + 手动按住语音按钮） */
...
/* ===== v5.5 第三后端：本地系统语音（localtts.m 提供；edge-tts 已被微软封锁 403） ===== */
...
static NSString *g_localVoice = nil;               /* AVSpeechSynthesisVoice identifier */
static NSString *const kLocalVoiceKey = @"TTSFloatLocalVoice";
static void LocalSaveVoice(NSString *vid) {
    g_localVoice = vid;
    [NSUserDefaults.standardUserDefaults setObject:g_localVoice forKey:kLocalVoiceKey];
}
...
static void QwenLoadState(void) {
    ...
    NSString *ev = [d stringForKey:kLocalVoiceKey];
    if (ev.length) g_localVoice = ev;
...
/* ===== v4.20: codec 参数读取 / PCM 重采样 —— 帧长必须与 QQ codec 配置一致 =====
 * 历史判决: v4.8~v4.19 一直按 16kHz/320 样本切帧喂 encode:withSamplesCount:callback:
 * 若 QQ codec 实际是 24kHz(帧 480)，每块喂 320 永远不足一帧 → 编码器缓冲饥饿 → 全 CNG 兜底
 * (与"每帧 2B 静音"现象完全吻合) */
static uint64_t CodecGetU(id obj, NSString *selName) {
    if (!obj) return 0;
    SEL s = NSSelectorFromString(selName);
    if (![obj respondsToSelector:s]) return 0;
    return ((uint64_t (*)(id, SEL))objc_msgSend)(obj, s);
}
static void CodecSetU(id obj, NSString *selName, uint64_t v) {
    if (!obj) return;
    SEL s = NSSelectorFromString(selName);
    if (![obj respondsToSelector:s]) return;
    ((void (*)(id, SEL, uint64_t))objc_msgSend)(obj, s, v);
}
static BOOL CodecCall(id obj, NSString *selName) {
    if (!obj) return NO;
    SEL s = NSSelectorFromString(selName);
    if (![obj respondsToSelector:s]) return NO;
    return ((BOOL (*)(id, SEL))objc_msgSend)(obj, s);
}
/* 线性插值重采样(16bit mono)；from==to 时原样返回 */
static NSData *ResamplePCM16(NSData *pcm, double from, double to) {
    if (!pcm.length || from <= 0 || to <= 0 || fabs(from - to) < 1.0) return pcm;
    const int16_t *src = (const int16_t *)pcm.bytes;
    NSUInteger n = pcm.length / 2;
    if (!n) return pcm;
    double ratio = to / from;
    NSUInteger m = (NSUInteger)((double)n * ratio);
    if (!m) return pcm;
    int16_t *dst = (int16_t *)malloc(m * sizeof(int16_t));
    if (!dst) return pcm;
    for (NSUInteger i = 0; i < m; i++) {
        double pos = (double)i / ratio;
        NSUInteger i0 = (NSUInteger)pos;
        double frac = pos - (double)i0;
        if (i0 >= n) i0 = n - 1;
        NSUInteger i1 = (i0 + 1 < n) ? (i0 + 1) : (n - 1);
        dst[i] = (int16_t)((double)src[i0] + ((double)src[i1] - (double)src[i0]) * frac);
    }
    NSData *out = [NSData dataWithBytes:dst length:m * 2];
    free(dst);
    return out;
}

/* ===== v4.20 注入相关全局 ===== */
static NSData *g_injectPcm = nil;         /* 注入用 PCM(已按 codec 采样率转换) */
static volatile BOOL g_injectActive = NO; /* UI「🎙 注入」开关, 默认关 */
static volatile NSUInteger g_injectPos = 0;
static volatile NSUInteger g_injectTotal = 0;
static NSData *g_lastTtsPcm = nil;        /* 最近一次合成的 PCM(注入源) */
static uint64_t g_realSampleRate = 16000; /* QQ 真实 codec 的采样率(hook 捕获) */
static volatile NSUInteger g_injectTail = 0;  /* 注入数据用完后静音续填块数 */

/* ===== v4.19: swizzle hook（修 v4.18 闪退） =====
 * ⚠️ v4.18 按住说话闪退根因（真机日志 4 段铁证）:
 *   QQSilkCodec.encode:withSamplesCount:callback: 的 type encoding = [Q40@0:8@16Q24@?32]
 *   → 返回类型是 Q (unsigned long long = 输出字节数), 不是对象!
 *   v4.18 的 hook 声明为 `id QQEncodeHook(...)` 且 `return ((id(*)(...))orig)(...)`:
 *   -fobjc-arc 下编译器按"返回 +0 autoreleased 对象"处理 → 对 x0 里的整数(如 40)
 *   执行 objc_autoreleaseReturnValue/objc_retain → 解引用非法地址 → SIGSEGV
 *   （日志: 每次 [codec-hook] self=.. count=7168 len=14336 head=.. 后紧跟 crash-guard init）
 * 修复: hook 返回 uint64_t, 转发用 (uint64_t(*)(id,SEL,id,uint64_t,id)) — ARC 零介入;
 *       日志改到主队列异步打印, 不在音频线程做文件 IO */
static id g_realCodec = nil;              /* QQ 真实录音用的 QQSilkCodec 实例(strong 保留) */
static uint64_t g_realCount = 0;          /* QQ 真实调用的 count 参数(每次喂入样本数) */
static unsigned char g_realPcmHead[16];   /* QQ 真实 PCM 前 16B(取证) */
static NSUInteger g_realPcmHeadLen = 0;
static char g_realHeadHex[40];            /* 上面 16B 的 hex(供日志) */
static volatile BOOL g_ttsEncoding = NO;  /* 我们 TTS 编码期间置 YES(hook 跳过记录) */
static NSUInteger g_hookLogs = 0;
static IMP g_origEncodeIMP = NULL;

static uint64_t QQEncodeHook(id self, SEL _cmd, id data, uint64_t count, id cb) {
    if (!g_ttsEncoding) {
        /* ===== v4.20 注入模式: 用我们的 PCM 顶替 QQ 录音数据 =====
         * QQ 自己编码 / 自己写 silk 文件 / 自己发送 → 格式 100% 原生可播放。
         * 由 UI「🎙 注入」按钮开关, 默认关闭(不影响真实录音)。 */
        if (g_injectActive && g_origEncodeIMP) {
            NSUInteger need = (NSUInteger)count * 2;
            NSData *src = g_injectPcm;
            NSMutableData *chunk = [NSMutableData dataWithLength:need ? need : 2];
            if (chunk) {
                NSUInteger pos = g_injectPos, total = src.length;
                if (src && pos < total) {
                    NSUInteger avail = total - pos;
                    if (avail > need) avail = need;
                    memcpy(chunk.mutableBytes, (const uint8_t *)src.bytes + pos, avail);
                }
                g_injectPos = pos + need;
                if (g_injectTotal && g_injectPos >= g_injectTotal) {
                    /* v4.21: 数据用完后继续填静音(而不是立刻关闭) —
                     * 否则用户按住时间超长时, 尾巴会混进真实录音;
                     * 续填 ~20 秒后自动关闭, 或用户再点按钮关闭 */
                    if (!g_injectTail) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            TTLog(@"[inject] 注入数据已用完(%lu 样本) — 续填静音防真实音尾, 再点按钮可关",
                                  (unsigned long)g_injectPos / 2);
                        });
                    }
                    g_injectTail++;
                    if (g_injectTail > 40) {
                        g_injectActive = NO;
                        NSUInteger tl = g_injectTail;
                        dispatch_async(dispatch_get_main_queue(), ^{
                            TTLog(@"[inject] 静音续填 %lu 块(≈20s) 后自动关闭", (unsigned long)tl);
                        });
                    }
                }
                return ((uint64_t (*)(id, SEL, id, uint64_t, id))g_origEncodeIMP)
                       (self, _cmd, (id)chunk, count, cb);
            }
        }
        g_realCodec = self;          /* strong 保留: QQ 初始化过的可用 codec 实例 */
        g_realCount = count;
        uint64_t srNow = CodecGetU(self, @"silkSampleRate");
        if (srNow) g_realSampleRate = srNow;
        if (g_hookLogs < 4) {
            g_hookLogs++;
            NSUInteger dl = 0;
            ...
            @try {
                ...
                dl = d ? d.length : 0;
                dp = d ? (const unsigned char *)d.bytes : NULL;
            } @catch (NSException *e) { dl = 0; dp = NULL; }
            NSUInteger hn = dl < 16 ? dl : 16;
            g_realPcmHeadLen = (dp ? hn : 0);
            if (g_realPcmHeadLen) memcpy(g_realPcmHead, dp, g_realPcmHeadLen);
            for (NSUInteger k = 0; k < g_realPcmHeadLen; k++)
                snprintf(g_realHeadHex + k * 2, 3, "%02x", g_realPcmHead[k]);
            g_realHeadHex[g_realPcmHeadLen * 2] = 0;
            NSUInteger l = dl;
            uint64_t c = count;
            id sself = self;   /* ARC 强引用, block 内 retain 安全 */
            dispatch_async(dispatch_get_main_queue(), ^{
                TTLog(@"[codec-hook] self=%p count=%llu len=%lu sr=%llu head=%s",
                      (__bridge void *)sself, c, (unsigned long)l, srNow, g_realHeadHex);
            });
        }
    }
    if (g_origEncodeIMP)
        return ((uint64_t (*)(id, SEL, id, uint64_t, id))g_origEncodeIMP)(self, _cmd, data, count, cb);
    return 0;   /* 兜底: 不返回垃圾, 不递归 */
}

/* ===== v4.21 探测: QQ 何时清空编码器状态 / recorder 实例(为"免按住说话"铺路) ===== */
static IMP g_origCleanIMP = NULL;
static IMP g_origInitEncIMP = NULL;
static BOOL g_cleanLogged = NO, g_initEncLogged = NO;
static BOOL QQCleanEncodeHook(id self, SEL _cmd) {
    if (!g_cleanLogged) {
        g_cleanLogged = YES;
        id sself = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            TTLog(@"[clean-hook] QQ 调用 cleanEncode self=%p ← 这就是 REAL 路全 CNG 的原因",
                  (__bridge void *)sself);
        });
    }
    if (g_origCleanIMP) return ((BOOL (*)(id, SEL))g_origCleanIMP)(self, _cmd);
    return NO;
}
static BOOL QQInitEncoderHook(id self, SEL _cmd) {
    if (!g_initEncLogged) {
        g_initEncLogged = YES;
        id sself = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            TTLog(@"[rec-hook] QQSIlkRecorder.initEncoder self=%p (官方编码器创建点)", (__bridge void *)sself);
        });
    }
    if (g_origInitEncIMP) return ((BOOL (*)(id, SEL))g_origInitEncIMP)(self, _cmd);
    return NO;
}
static void InstallProbeHooks(void) {
    Class cc = NSClassFromString(@"QQSilkCodec");
    if (cc) {
        Method m = class_getInstanceMethod(cc, NSSelectorFromString(@"cleanEncode"));
        if (m) { g_origCleanIMP = method_setImplementation(m, (IMP)QQCleanEncodeHook);
                 TTLog(@"[clean-hook] installed"); }
        else TTLog(@"[clean-hook] sel MISS");
    }
    Class rc = NSClassFromString(@"QQSilkRecorder");
    if (rc) {
        Method m = class_getInstanceMethod(rc, NSSelectorFromString(@"initEncoder"));
        if (m) { g_origInitEncIMP = method_setImplementation(m, (IMP)QQInitEncoderHook);
                 TTLog(@"[rec-hook] installed"); }
        else TTLog(@"[rec-hook] sel MISS");
    }
}

static void InstallCodecHook(void) {
    Class c = NSClassFromString(@"QQSilkCodec");
    if (!c) { TTLog(@"[codec-hook] QQSilkCodec MISS"); return; }
    SEL s = NSSelectorFromString(@"encode:withSamplesCount:callback:");
    Method m = class_getInstanceMethod(c, s);
    if (!m) { TTLog(@"[codec-hook] method MISS"); return; }
    const char *enc = method_getTypeEncoding(m);
    char rt = enc ? enc[0] : '?';
    TTLog(@"[codec-hook] types=%s return=%c (期望 Q=字节数)", enc ? enc : "(null)", rt);
    if (rt != 'Q' && rt != 'q' && rt != 'L' && rt != 'I' && rt != 'v')
        TTLog(@"[codec-hook] ⚠️ 返回类型非整数/void — 转发仍按 uint64 处理(丢弃对象返回值, 不崩)");
    g_origEncodeIMP = method_setImplementation(m, (IMP)QQEncodeHook);
    TTLog(@"[codec-hook] installed (v4.19 修 ARC 返回语义)");
}

/* 分帧编码: 每帧 [u16LE len][frame] 打包, frameSamples = QQ codec 的每帧样本数
 * 返回 voice 帧数(帧长>4B)；outMaxLen/outSumLen 供判决 */
static NSUInteger SilkEncodeFrames(id codec, NSData *pcm, NSUInteger frameSamples,
                                   NSUInteger maxFrames, NSMutableData *outPacked,
                                   NSUInteger *outFrames, NSUInteger *outCbs,
                                   NSUInteger *outMaxLen, NSUInteger *outSumLen) {
    SEL cbSel = NSSelectorFromString(@"encode:withSamplesCount:callback:");
    if (!codec || !frameSamples) return 0;
    if (![codec respondsToSelector:cbSel]) return 0;
    ...
    NSUInteger nF = 0, voice = 0, maxLen = 0, sumLen = 0;
    __block NSUInteger cbs = 0;
    for (NSUInteger off = 0; off + frameSamples <= total && nF < maxFrames; off += frameSamples) {
        @autoreleasepool {
            NSData *chunk = [NSData dataWithBytes:samples + off length:frameSamples * 2];
            uint64_t before = lastN;
            ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
                (codec, cbSel, chunk, (NSUInteger)frameSamples, ^(const void *d, uint64_t n) {
                    if (!d || !n) return;
                    lastD = d; lastN = n; cbs++;
                });
            if (lastN > before && lastD) {
                NSUInteger flen = (NSUInteger)(lastN - before);
                if (flen > 65535) flen = 65535;
                uint16_t fl = (uint16_t)flen;
                [outPacked appendBytes:&fl length:2];
                [outPacked appendBytes:((const unsigned char *)lastD + before) length:flen];
                if (flen > 4) voice++;
    ...
    if (outFrames) *outFrames = nF;
    if (outCbs) *outCbs = cbs;
    if (outMaxLen) *outMaxLen = maxLen;
    if (outSumLen) *outSumLen = sumLen;
    return voice;
}
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        NSUInteger total = pcm.length / 2;
        {
            NSUInteger above = 0; long long sumSq = 0; int16_t peak = 0;
            ...
            double rms = sqrt((double)sumSq / (total ? total : 1));
            TTLog(@"[pcm] rms=%.0f peak=%d |v|>1000:%lu%%", rms, peak, (unsigned long)(above * 100 / (total ? total : 1)));
        }

        /* ===== v4.20: 用 QQ codec 的真实配置(采样率/每帧样本数)来切帧 =====
         * v4.8~v4.19 全部实验都按 16kHz/320 样本切帧 → 若 codec 实为 24kHz(480/帧),
         * 每块喂入永远不足一帧 → 编码器饥饿 → 全 2B CNG 帧(历史全部"静音"结论的根源) */
        uint64_t srCodec = g_realSampleRate;
        if (!srCodec && g_realCodec) srCodec = CodecGetU(g_realCodec, @"silkSampleRate");
        if (!srCodec) srCodec = 16000;
        uint64_t brCodec = g_realCodec ? CodecGetU(g_realCodec, @"silkBitRate") : 0;
        uint64_t spfCodec = g_realCodec ? CodecGetU(g_realCodec, @"silkSamplesPerFrame") : 0;
        uint64_t dpfCodec = g_realCodec ? CodecGetU(g_realCodec, @"silkDurationPreFrame") : 0;
        if (!spfCodec) spfCodec = srCodec * 20 / 1000;
        if (!brCodec) brCodec = 24000;
        TTLog(@"[silk] codec配置 sr=%llu br=%llu spf=%llu dpf=%llu → 帧=%llu样本",
              srCodec, brCodec, spfCodec, dpfCodec, spfCodec);

        NSData *pcmFit = ResamplePCM16(pcm, (double)rate, (double)srCodec);
        if (pcmFit != pcm)
            TTLog(@"[silk] PCM 重采样 %uHz→%lluHz: %luB→%luB", rate, srCodec,
                  (unsigned long)pcm.length, (unsigned long)pcmFit.length);

        NSUInteger frameSamples = (NSUInteger)spfCodec;
        NSMutableData *packR = [NSMutableData data], *packR2 = [NSMutableData data], *packO = [NSMutableData data];
        NSUInteger fR = 0, cR = 0, vR = 0, mR = 0, sR = 0;
        NSUInteger fR2 = 0, cR2 = 0, vR2 = 0, mR2 = 0, sR2 = 0;
        NSUInteger fO = 0, cO = 0, vO = 0, mO = 0, sO = 0;

        /* R1: QQ 真实录音的 codec 实例, 原样全量编码 */
        if (g_realCodec) {
            g_ttsEncoding = YES;
            vR = SilkEncodeFrames(g_realCodec, pcmFit, frameSamples, NSUIntegerMax,
                                  packR, &fR, &cR, &mR, &sR);
            g_ttsEncoding = NO;
            TTLog(@"[silk] R1 REAL原样: cbs=%lu frames=%lu voice=%lu max=%lu avg=%lu out=%luB",
                  (unsigned long)cR, (unsigned long)fR, (unsigned long)vR, (unsigned long)mR,
                  (unsigned long)(fR ? sR / fR : 0), (unsigned long)packR.length);
        } else {
            TTLog(@"[silk] R1 跳过 — 无真实 codec(先在QQ里真实发一条语音)");
        }

        /* R2: REAL + C3 重设参数(v4.15 判决的安全组合, 编码后恢复 QQ 原值) */
        if (g_realCodec) {
            uint64_t sr0 = CodecGetU(g_realCodec, @"silkSampleRate");
            uint64_t br0 = CodecGetU(g_realCodec, @"silkBitRate");
            CodecCall(g_realCodec, @"setEncodeParam");
            CodecSetU(g_realCodec, @"setSilkSampleRate:", srCodec);
            CodecSetU(g_realCodec, @"setSilkBitRate:", 24000);
            g_ttsEncoding = YES;
            vR2 = SilkEncodeFrames(g_realCodec, pcmFit, frameSamples, NSUIntegerMax,
                                   packR2, &fR2, &cR2, &mR2, &sR2);
            g_ttsEncoding = NO;
            CodecCall(g_realCodec, @"setEncodeParam");
            if (sr0) CodecSetU(g_realCodec, @"setSilkSampleRate:", sr0);
            if (br0) CodecSetU(g_realCodec, @"setSilkBitRate:", br0);
            TTLog(@"[silk] R2 REAL+C3: cbs=%lu frames=%lu voice=%lu max=%lu avg=%lu out=%luB (已恢复 sr=%llu br=%llu)",
                  (unsigned long)cR2, (unsigned long)fR2, (unsigned long)vR2, (unsigned long)mR2,
                  (unsigned long)(fR2 ? sR2 / fR2 : 0), (unsigned long)packR2.length, sr0, br0);
        }

        /* O1: 自建实例对照(历史结论: 必全静音) */
        @try {
            id own = [[c alloc] init];
            CodecCall(own, @"setEncodeParam");
            CodecSetU(own, @"setSilkSampleRate:", srCodec);
            CodecSetU(own, @"setSilkBitRate:", 24000);
            g_ttsEncoding = YES;
            vO = SilkEncodeFrames(own, pcmFit, frameSamples, NSUIntegerMax, packO, &fO, &cO, &mO, &sO);
            g_ttsEncoding = NO;
            TTLog(@"[silk] O1 OWN: cbs=%lu frames=%lu voice=%lu max=%lu avg=%lu out=%luB",
                  (unsigned long)cO, (unsigned long)fO, (unsigned long)vO, (unsigned long)mO,
                  (unsigned long)(fO ? sO / fO : 0), (unsigned long)packO.length);
        } @catch (NSException *e) { g_ttsEncoding = NO; TTLog(@"[silk] O1 异常 %@", e); }

        /* 选路: voice 帧数优先, 平局取输出字节大者 */
        NSMutableData *full = packR;
        const char *win = "R1-REAL";
        NSUInteger vBest = vR, lBest = packR.length;
        if (vR2 > vBest || (vR2 == vBest && packR2.length > lBest)) {
            full = packR2; win = "R2-REAL-C3"; vBest = vR2; lBest = packR2.length;
        }
        if (vO > vBest || (vO == vBest && packO.length > lBest)) {
            full = packO; win = "O1-OWN"; vBest = vO; lBest = packO.length;
        }
        TTLog(@"[silk] 胜出 %s (voice=%lu 帧, %luB)", win, (unsigned long)vBest, (unsigned long)full.length);
        if (full.length == 0) { TTLog(@"[silk] 全量编码失败"); return nil; }

        NSMutableData *file = [NSMutableData data];
        if (g_realHeadLen >= 10) [file appendBytes:g_realHead length:10];
        else [file appendBytes:"#!SILK_V3" length:9];
        [file appendData:full];
        TTLog(@"[silk] silk文件共 %luB (帧数据 %luB)", (unsigned long)file.length, (unsigned long)full.length);
        return file;
 ...
 *   touchDown → 等 N 秒 → touchUpInside
 * 此时 v4.21 的注入已在待命 → QQ 录制到的就是我们的 PCM → 松手发出去的语音条 = TTS
 * 找不到按钮则退回手动提示(注入仍有效)。 */
static BOOL g_autoPttEnabled = NO;   /* v4.24: 停用自动按住(QQPttRecordBtn 只吃真实触摸, sendActions 无效) */
static NSUInteger g_autoPttTried = 0;

static BOOL TTIsOurWindow(UIWindow *w) {
    if (!w) return NO;
    if (g_ttsWindow && w == g_ttsWindow) return YES;
    const char *cn = class_getName(w.class);
    return cn && strstr(cn, "TTSPassWindow") != NULL;
}

/* v4.23: 遍历【所有】窗口(排除我们自己的浮窗) —— v4.22 只从 keyWindow(我们的) 开始,
 * 结果候选只有我们自己面板的 4 个按钮, 根本看不到 QQ 的控件。
 * 匹配范围扩大到所有 UIView 的 title/text/accessibilityLabel/Identifier,
 * 命中非 UIControl 时向上找最近的可驱动 UIControl。 */
static UIView *TTFindVoiceButton(NSMutableArray *outLabels) {
    NSArray *wins = UIApplication.sharedApplication.windows;
    UIView *hit = nil;
    NSUInteger guard = 0;
    for (UIWindow *w in wins) {
        if (TTIsOurWindow(w) || w.hidden) continue;
        NSMutableArray *queue = [NSMutableArray arrayWithObject:w];
        while (queue.count && guard++ < 30000) {
            UIView *v = queue.firstObject;
            [queue removeObjectAtIndex:0];
            NSString *t = @"";
            if ([v isKindOfClass:[UIButton class]]) t = ((UIButton *)v).currentTitle ?: @"";
            else if ([v isKindOfClass:[UILabel class]]) t = ((UILabel *)v).text ?: @"";
            NSString *a = v.accessibilityLabel ?: @"";
            NSString *i = v.accessibilityIdentifier ?: @"";
            NSString *all = [NSString stringWithFormat:@"%@|%@|%@", t, a, i];
            BOOL shortish = (t.length <= 12 && a.length <= 20 && i.length <= 20);
            if (outLabels && shortish && all.length > 2 && outLabels.count < 40)
                [outLabels addObject:[NSString stringWithFormat:@"%@{%@}", NSStringFromClass(v.class), all]];
            if (!hit && all.length > 2 && shortish &&
                ([all containsString:@"按住"] || [all containsString:@"说话"])) {
                UIView *cand = v;
                if (![cand isKindOfClass:[UIControl class]]) {
                    UIView *p = cand.superview;
                    for (int k = 0; k < 4 && p; k++, p = p.superview)
                        if ([p isKindOfClass:[UIControl class]]) { cand = p; break; }
                }
                hit = cand;
            }
...
- (void)injectTapped;                    /* v4.20: 🎙 注入开关 */
- (void)armInjectionWithPcm:(NSData *)pcm tag:(NSString *)tag;   /* v4.22: 注入待命统一入口 */
- (void)autoPttForSeconds:(double)sec;                            /* v4.22: 自动按住说话 */
    ...
    obs = [[NSNotificationCenter defaultCenter]
        ...
        usingBlock:^(NSNotification *n) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)),
                ...
                if (WXChainIsWeChatBundle()) WXChainInstallHooks();   /* v5.0: 微信链 hooks */
...
- (void)togglePanel {
    ...
    if (g_backend == 2) {
        self.voiceLabel.text = [NSString stringWithFormat:@"[本地] %@ %.2fx", LocalDisplayForID(g_localVoice), g_qwenRate];
    } else if (g_backend == 1) {
...
- (void)showVoiceList {
    ...
    NSString *title;
    if (g_backend == 1) {
        title = [NSString stringWithFormat:@"选择音色（千问48 + 本地%lu + 原%lu）", (unsigned long)LocalVoiceCount(), (unsigned long)n];
    } else {
...
/* v4.22: 注入待命统一入口(自动超时 + 可选自动按住说话) */
- (void)armInjectionWithPcm:(NSData *)pcm tag:(NSString *)tag {
    if (!pcm.length) { self.statusLabel.text = @"没有可注入的 PCM（先合成一条）"; return; }
    uint64_t sr = g_realSampleRate ? g_realSampleRate : 16000;
    NSData *conv = ResamplePCM16(pcm, (double)g_targetSampleRate, (double)sr);
    g_injectPcm = conv;
    g_injectTotal = conv.length;
    g_injectPos = 0;
    g_injectTail = 0;
    g_injectActive = YES;
    double sec = (double)conv.length / 2.0 / (double)sr;
    TTLog(@"[inject] arm(%@): %luB @%lluHz ≈ %.2fs", tag, (unsigned long)conv.length, sr, sec);
    self.statusLabel.text = [NSString stringWithFormat:@"✔ 已合成 %.1f 秒 → 回聊天按住语音按钮约 %.1f 秒后松手", sec, sec];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(60 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (g_injectActive && g_injectPos == 0) {
            g_injectActive = NO;
            g_injectPcm = nil;
            self.statusLabel.text = @"注入待命 60 秒未使用，已自动取消";
            TTLog(@"[inject] 待命 60s 未使用 → 自动取消");
        }
    });
    if (g_autoPttEnabled) [self autoPttForSeconds:sec];
}

/* v4.22: 自动按住说话 —— 替用户按住 QQ 的「按住说话」按钮 */
- (void)autoPttForSeconds:(double)sec {
    g_autoPttTried++;
    NSMutableArray *labels = [NSMutableArray array];
    UIView *hit = TTFindVoiceButton(labels);
    if (!hit) {
        TTLog(@"[auto-ptt] 未找到「按住说话」控件 — 请手动按住 %.1fs", sec);
        TTLog(@"[auto-ptt] 候选(非我们窗口, %lu): %@", (unsigned long)labels.count,
              labels.count ? [labels componentsJoinedByString:@" "] : @"(无)");
        self.statusLabel.text = [NSString stringWithFormat:@"请手动按住说话约 %.1f 秒后松手", sec];
        return;
    }
    TTLog(@"[auto-ptt] 命中 %@ a11y=「%@」→ 自动按住 %.1fs",
          NSStringFromClass(hit.class), hit.accessibilityLabel ?: @"", sec);
    if (![hit isKindOfClass:[UIControl class]]) {
        TTLog(@"[auto-ptt] ⚠️ 命中控件不是 UIControl — 无法用 target-action 驱动");
        TTLog(@"[auto-ptt] 候选(%lu): %@", (unsigned long)labels.count,
              [labels componentsJoinedByString:@" "]);
        self.statusLabel.text = [NSString stringWithFormat:@"请手动按住说话约 %.1f 秒后松手", sec];
        return;
    }
    UIControl *ctl = (UIControl *)hit;
    self.statusLabel.text = [NSString stringWithFormat:@"自动按住说话 %.1f 秒…", sec];
    [ctl sendActionsForControlEvents:UIControlEventTouchDown];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((sec + 0.35) * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        [ctl sendActionsForControlEvents:UIControlEventTouchUpInside];
        TTLog(@"[auto-ptt] 已抬起(发送)");
        self.statusLabel.text = @"✔ 已自动发送，去聊天看语音条";
    });
}

/* v4.20: 注入开关 —— 开: 下次按住说话发出的就是这段 TTS 语音; 再点一次关闭 */
- (void)injectTapped {
    if (g_injectActive) {
        g_injectActive = NO;
        g_injectPcm = nil;
        g_injectPos = 0;
        g_injectTotal = 0;
        self.statusLabel.text = @"注入已关闭（恢复正常录音）";
        TTLog(@"[inject] off");
        return;
    }
    NSData *pcm = g_lastTtsPcm;
    if (!pcm.length) {
        self.statusLabel.text = @"先点左侧「合成语音」生成一条";
        return;
    }
    [self armInjectionWithPcm:pcm tag:@"manual"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    ...
    if (g_voiceFilter) {
        ...
        isEdge = [name hasPrefix:@"本地·"];
        isQwen = [self qwIsQwenDisplay:name];
        if (isEdge) for (NSUInteger i = 0; i < LocalVoiceCount(); i++) if ([name isEqualToString:LocalVoiceDisplay(i)]) { edIdx = i; break; }
    ...
    else if (isEdge) selected = (g_backend == 2 && [LocalVoiceID(edIdx) isEqualToString:g_localVoice]);
...
- (void)searchBar:(UISearchBar *)sb textDidChange:(NSString *)text {
    ...
    if (q.length == 0) {
        ...
        for (NSUInteger i = 0; i < LocalVoiceCount(); i++) {
            NSString *dn = LocalVoiceDisplay(i);
            if ([dn rangeOfString:q options:NSCaseInsensitiveSearch].location != NSNotFound ||
                [LocalVoiceID(i) rangeOfString:q options:NSCaseInsensitiveSearch].location != NSNotFound) [r addObject:dn];
        }
        for (NSUInteger i = 0; i < QW_VOICE_COUNT; i++) {
...
- (void)sendDirect {
    ...
    void (^onAudio)(NSData *, NSError *) = ^(NSData *audio, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            g_lastTtsPcm = pcm;   /* v4.20: 供「🎙 注入」使用 */
            if (!pcm) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.send.enabled = YES; [self.spinner stopAnimating];
             ...
             * maxAbs<100 ≈ 全静音 → AVAudioConverter 链有问题(非 codec 采样率) */
            {
                ...
                TTLog(@"[qq] PCM %luB ≈ %lums maxAbs=%lu head32B=%s",
                      (unsigned long)pcm.length, (unsigned long)ms,
                      (unsigned long)maxAbs, hh);
            }

            /* ===== v4.21: 合成完成 → 自动进入「🎙 注入待命」 =====
             * 不再自己编码: v4.20 日志实证 QQ 的 codec 实例在录音结束后被 cleanEncode 清状态,
             * R1/R2/O1 三路全量编码 avg=2B/帧(全 CNG) → 发出去就是"1 秒静音录音"
             * (codec配置 sr=16000 br=16000 spf=320 → 帧长本来就是对的, 根因是编码器状态)
             * 改走已验证可行路径: 我们的 PCM 顶替 QQ 录音数据 → QQ 自己编码/自己发送 */
            {
                ...
                TTLog(@"[qq] v4.22 合成完成 → 注入待命 + 自动按住说话: %luB ≈ %.2fs",
                      (unsigned long)pcm.length, sec);
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.send.enabled = YES; [self.spinner stopAnimating];
                    [self armInjectionWithPcm:pcm tag:@"auto"];
                });
            }
            return;
            /* ↓↓↓ v4.21 起以下旧链不再执行(自己编码 avg=2B/帧 全静音 + 覆写重放) ↓↓↓ */
    ...
    if (g_backend == 2) {
        ...
        TTLog(@"[local] 请求 voice=%@ rate=%.2f len=%lu", g_localVoice, g_qwenRate, (unsigned long)text.length);
        if (!g_localVoice.length) {
            /* 首次使用：默认取列表第一个 */
            if (LocalVoiceCount() > 0) { g_localVoice = LocalVoiceID(0); LocalSaveVoice(g_localVoice); }
        }
        RequestLocalTTS(text, g_localVoice, g_qwenRate, onAudio);
    } else if (g_backend == 1) {
...
static void QQFloatV2Init(void) {
    ...
    LocalSetLogPath(g_logPath);        /* v5.5: 本地合成共用同一日志文件（行首 [LOCAL] 区分） */
    g_isWeChatHost = WXChainIsWeChatBundle();
    if (g_isWeChatHost) {
        /* 微信宿主：不装 QQ hooks（类都不存在）；微信链 hooks 在启动通知 +3s 后安装 */
        TTLog(@"QQFloat v5.5 init — **微信宿主**（+本地语音后端）");
        return;
    }
    InstallCodecHook();
    InstallProbeHooks();
    TTLog(@"QQFloat v5.0 init — QQ 宿主（注入待命 + 手动按住语音按钮）");
