/*
 * QQFloat_v41.m — v4.1: v4.0 日志 + CrashLog 判决（QQFloat_1.log 55023B + QQFloatCrash.log）:
 *   ✅ dlsym 垃圾命中根除（StringMetaAcc/String.Type 不再打 — tier 全 MISS）
 *   ❌ bridgeSym=0x0 — 带 _ 前缀符号在 RTLD_DEFAULT 下也 MISS!
 *      → 真因: two-level namespace 下 RTLD_DEFAULT 不遍历全部 image 导出表;
 *        libswiftFoundation.dylib 虽已加载(QQ LC_LOAD_DYLIB 依赖表实锤), 
 *        但 dlsym(RTLD_DEFAULT) 找不到它的 Foundation 桥符号
 *   ❌ 空串兜底 (q0=0/q1=0) 落地后 sendPttMsg 崩:
 *      [CRASH] sig=11 addr=0x18 ×6 (同偏移, 每次重试都崩同一点)
 *      → addr=0x18 = 结构偏移 0x18 解引用 nil — sendPtt 深层消费
 *        String q1=0 (object 槽空) 或 placeholderMsgInfo=nil
 *
 * v4.1 三处修复:
 *   1.【dlopen 显式句柄】RTLD_DEFAULT 全 MISS 根因 = two-level namespace。
 *      改 dlopen("/usr/lib/swift/libswiftFoundation.dylib", RTLD_LAZY|RTLD_NOLOAD)
 *      拿显式 handle → dlsym(handle, "_$sSS10FoundationE36_...")
 *      String metadata accessor "_$sSSMa" 在 libswiftCore.dylib — 同法另开 handle。
 *      RTLD_NOLOAD: 已加载则只加引用计数, 不真正重载 — 零风险。
 *   2.【补 placeholderMsgInfo】v3.4~v4.0 一直传 nil!
 *      sendPtt 内部把 placeholderMsgInfo 塞进占位气泡元素, nil → 深层解引用崩。
 *      AudioPlaceholderMsgInfo 同 model 路线: class_createInstance + memset
 *      (v3.2 实锤 createInstance 对这类 Swift 桥类不崩 — qq-scan 12 万类扫过安然无恙)
 *      elementID 槽直写 0 (合法 int64)
 *   3.【String 四参桥保留 + 位型记录】桥命中后照 v4.0 四参 ABI (x0=meta,x2=arg,w3=1)
 *      — 全 MISS 时不再兜空串直发(已实证崩), 改为【跳过发送】+ 明确 WARN 日志
 *      → 宁可不发不崩, 下轮日志拿 [qq-real] 真实位型后硬编码
 *
 * v4.1 判决树（下次日志）:
 *   - "[qq] swfHandle=0x... sym=0x..." (dlopen+dlsym 命中) → 四参桥 → q0/q1 位型
 *   - q1 高 8 位 0xD0/0x90 (堆对象) / q0 高字节 0xE0 (small) → String 真落地 → 全链闭环
 *   - swfHandle 命中但四参桥异常 → 看异常串
 *   - 仍 MISS → "⚠️ String 桥全 MISS — 已跳过发送(不崩)" → 用户真实录一条语音
 *     → D 钩子 [qq-real] raw16 = 下版硬编码依据
 *   - [CRASH] addr=0x18 消失 = placeholderMsgInfo 修复生效
 *
 * ====================== 历史判决存档 ======================
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            @try {
                ...
                if (!audioModel) { TTLog(@"[qq] class_createInstance 返回 nil"); }
                    ...
                    /* v3.6 新增: class_createInstance 不清零内存(Swift String 16B 是垃圾)
                     * 先整块清零 instanceSize — 空串 (q0=0,q1=0) 是合法 Swift String,
                     * 枚举 ivar 清零 = 0 也安全 */
                    memset((__bridge void *)audioModel, 0, class_getInstanceSize(mCls));
                    /* v3.6: 静态铁证 — 本类方法表只有 init/.cxx_destruct, 无任何 setter
                     * (v3.5 respondsToSelector MISS 实锤; 0x106e1f94c 那批 setter 属 NTSendableAudioModel)
                     * object_setIvar 全废: audioType 槽 size=1 会被 8B 指针污染 +8..+0xF;
                     *   audioDuration 槽 size=4(float) 同理 → sendPtt 读出垃圾 float
                     * 全部改按 ivar size 裸内存直写:
                     ...
                     * v3.8: 三优先级产 String, 彻底不猜 ABI:
                     *   P1 借 NTSendableAudioModel 官方 setAudioFilePath:(静态实锤 setter 存在,
                     *      imp 0x106e1fb48 内部调桥) → 拷贝其 +0x10 16B 槽位
                     *   P2 dlsym 桥(v4.0 四参 ABI + 全符号带下划线) + 位型判定
                     *   P3 small-string 手工布局(≤15B); 全失败 → 空串(0,0) + WARN
                     *   v3.7 崩溃根因: borrowed 布局字段序反了 — q1=[+0x18] 才是 object 侧
                     *   (sendPtt 0x10862fa7c bridgeObjectRetain([+0x18]) + .cxx_destruct
                     *    release [+0x18] 双实锤), v3.7 把 NSString 指针写进了 q0(count/flags 槽) */
                    uint64_t sq0 = 0, sq1 = 0;
                    BOOL stringOK = NO;

                    /* v3.9: P1(借 NTSendable setter) 判决性移除 — v3.8 真机日志实锤崩点:
                     * 日志停在 sendCls=INTAIOChatProtocol.NTSendableAudioModel, 后续
                     * lend/responds/P1 全部没打 → 崩在 class_createInstance 或
                     * respondsToSelector 触发的 Swift 类 realize(metadata accessor 深初始化)
                     * 【与 v3.7 ProtobufLite NoClass 同模式: 提前 realize 未就绪的 Swift/桥类】
                     * 静态佐证: NTSendable ro(flags=0x81) 方法表/ivar 表全空, 完全靠 Swift
                     * metadata 运行时挂 — 触碰即走 accessor。qq-scan 扫 12 万类不崩是因为
                     * respondsToSelector 前的 realize 时机不同(启动 5s 后注册表已就绪,
                     * 而发送时刻我们主动 create 的 realize 走了不同路径)
                     * P2/P3 保留 — P2 dlsym 桥在 v3.6 真机曾命中非零符号(0x1855cddf8) */

                    /* ---- P2: 桥 (v4.1: dlopen 显式句柄 + 四参 ABI) — 唯一首选 ----
                     * v4.0 实锤: 带 _ 前缀 dlsym(RTLD_DEFAULT) 仍全 MISS
                     * (two-level namespace 不遍历 libswiftFoundation 导出表)
                     * → dlopen 拿显式句柄再 dlsym。RTLD_NOLOAD=已加载只加引用计数 */
                    {
                        typedef struct { uint64_t a; uint64_t b; } BridgeRet;
                        void *swfHandle = dlopen("/usr/lib/swift/libswiftFoundation.dylib",
                                                 RTLD_LAZY | RTLD_NOLOAD);
                        void *swcHandle = dlopen("/usr/lib/swift/libswiftCore.dylib",
                                                 RTLD_LAZY | RTLD_NOLOAD);
                        TTLog(@"[qq] swfHandle=%p swcHandle=%p", swfHandle, swcHandle);
                        void *bridgeSym = NULL;
                        if (swfHandle) {
                            bridgeSym = dlsym(swfHandle,
                                "_$sSS10FoundationE36_unconditionallyBridgeFromObjectiveCySSSo8NSStringCSgFZ");
                            if (!bridgeSym)
                                bridgeSym = dlsym(swfHandle, "_unconditionallyBridgeFromObjectiveC");
                        }
                        if (!bridgeSym) {
                            /* 回退: RTLD_DEFAULT 全局再试一遍(某些越狱环境 flat namespace) */
                            bridgeSym = dlsym(RTLD_DEFAULT,
                                "_$sSS10FoundationE36_unconditionallyBridgeFromObjectiveCySSSo8NSStringCSgFZ");
                        }
                        TTLog(@"[qq] bridgeSym=%p", bridgeSym);
                        if (bridgeSym) {
                            /* String metadata accessor: String 是 stdlib 类型 → libswiftCore */
                            void *metaAcc = swcHandle ? dlsym(swcHandle, "_$sSSMa") : NULL;
                            if (!metaAcc) metaAcc = dlsym(RTLD_DEFAULT, "_$sSSMa");
                            ...
                            if (metaAcc) {
                                ...
                                @try {
                                    ...
                                    if (strType) {
                                        /* v4.0 四参 ABI: (x0=String.Type, x1=不读,
                                         * x2=NSString 指针, w3=1) — 0x106e1fb40-50 实锤 */
                                        BridgeRet (*br4)(id, id, id, int) =
                                            (BridgeRet (*)(id, id, id, int))bridgeSym;
                                        BridgeRet r4 = br4(strType, (id)0, silkPath, 1);
                                        sq0 = r4.a; sq1 = r4.b;
                                        TTLog(@"[qq] 四参桥 q0=%#llx q1=%#llx",
                                              (unsigned long long)sq0, (unsigned long long)sq1);
                                    }
                                } @catch (NSException *e) {
                                    TTLog(@"[qq] metatype/四参桥异常 %@", e);
                                }
                            }
                            /* 位型判定: 合法 Swift String(q0=count/flags, q1=object) */
                    ...
                    /* P3 已并入下方 else 兜底块（v4.1: 空串证伪后重写） */

                    if (stringOK) {
                        const char *base = (const char *)(__bridge void *)audioModel;
                        memcpy((void *)(base + 0x10), &sq0, 8);   /* q0 = count/flags */
                        memcpy((void *)(base + 0x18), &sq1, 8);   /* q1 = object — v3.7 反了的修复 */
                        TTLog(@"[qq] String 已落地 (q0=count q1=object 正序)");
                    } else {
                        /* v4.1 判决性变更: 空串兜底已被 v4.0 真机证伪
                         * (q0=0/q1=0 落地 → sendPtt 深层 [nil+0x18] SIGSEGV addr=0x18 ×6)
                         * → 全 MISS 时写 small-string 占位(不崩不发) + 跳过发送
                         * 等 D 钩子 [qq-real] 真实位型后硬编码 */
                        const char *fp = silkPath.fileSystemRepresentation;
                        size_t fl = strlen(fp);
                        if (fl <= 15) {
                            uint8_t buf[16]; memset(buf, 0, 16);
                            memcpy(buf, fp, fl);
                            memcpy(&sq0, buf, 8);
                            uint8_t hi = (uint8_t)(0xE0 | (uint8_t)fl);
                            uint64_t t = 0; memcpy(&t, buf + 8, 7);
                            sq1 = t | ((uint64_t)hi << 56);
                            TTLog(@"[qq] P3 small-string 手工布局 len=%zu q0=%#llx q1=%#llx",
                                  fl, (unsigned long long)sq0, (unsigned long long)sq1);
                        } else {
                            sq0 = 0; sq1 = 0;
                            TTLog(@"[qq] ⚠️ String 桥全 MISS 且路径>15B — 跳过发送(不崩)");
                        }
                        const char *base = (const char *)(__bridge void *)audioModel;
                        ...
                        if (fl > 15) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                self.send.enabled = YES; [self.spinner stopAnimating];
                                self.statusLabel.text = @"String桥MISS—先真实录条语音看[qq-real]日志";
                            });
                            return;
                        }
                    }
                    /* audioType: uint8 槽(size=1) — 1=silk【推测,按 sendResult 判决】 */
                    {
                        const char *base = (const char *)(__bridge void *)audioModel;
                        uint8_t t = 1;
                        memcpy((void *)(base + 8), &t, 1);
                    }
                    /* audioDuration: float 槽(size=4) — 毫秒直写 float */
                    {
                        const char *base = (const char *)(__bridge void *)audioModel;
                        float dms = (float)ms;
                        memcpy((void *)(base + 0x20), &dms, 4);
                    }
                    /* placeholderMsgType@+0x41 / isAIVoice@+0x42: uint8 槽 */
                    {
                        const char *base = (const char *)(__bridge void *)audioModel;
                        uint8_t ph = 0, ai = 0;
                        memcpy((void *)(base + 0x41), &ph, 1);
                        memcpy((void *)(base + 0x42), &ai, 1);
                    }
                    /* 回读全部关键槽 — 判决用 */
                    {
                        const char *base = (const char *)(__bridge void *)audioModel;
                        uint64_t q0 = *(const uint64_t *)(base + 0x10);
                        uint64_t q1 = *(const uint64_t *)(base + 0x18);
                        uint8_t  t  = *(const uint8_t  *)(base + 8);
                        float    fd = *(const float    *)(base + 0x20);
                        uint32_t durBits = *(const uint32_t *)(base + 0x20);
                        TTLog(@"[qq] AudioModel ok (直写, type=%u durF=%.1f durBits=%#x, String q0=%#llx q1=%#llx, ct=1 dur=%lu)",
            ...
            /* v4.1: 补 placeholderMsgInfo — v3.4~v4.0 一直传 nil, sendPtt 深层
             * 解引用 nil 崩 (addr=0x18 ×6)。AudioPlaceholderMsgInfo 走 createInstance
             * + memset 同 model 路线 (v3.2 实锤这类 Swift 桥类 createInstance 不崩)。
             * ivar: elementID(int64) 清零 = 0 合法 */
            id placeholderInfo = nil;
            {
                Class pCls = NSClassFromString(@"_TtC10MsgManager23AudioPlaceholderMsgInfo");
                if (pCls) {
                    @try {
                        placeholderInfo = (id)class_createInstance(pCls, 0);
                        if (placeholderInfo) {
                            memset((__bridge void *)placeholderInfo, 0, class_getInstanceSize(pCls));
                            TTLog(@"[qq] placeholderInfo ok (%p, size=%zu)",
                                  (__bridge void *)placeholderInfo,
                                  class_getInstanceSize(pCls));
                        }
                    } @catch (NSException *e) {
                        TTLog(@"[qq] placeholderInfo 构造异常 %@", e);
                        placeholderInfo = nil;
                    }
                } else TTLog(@"[qq] AudioPlaceholderMsgInfo 类 MISS — 传 nil");
            }
            ...
            /* v3.6: 细化 sendPtt 崩溃定位 — 调用瞬间前后双日志 + 调用后 300ms 回读 filePath 槽
             * blocks 仍传 nil (v3.4 反汇编: x21/x19 各过 _Block_copy, nil 走 cbz 跳过)
             * v4.1: placeholderMsgInfo 改传真实实例 (nil 已实证 addr=0x18 崩) */
            @try {
                TTLog(@"[qq] sendPttMsg 调用前 (blocks=nil, model=%p, ph=%p)",
                      (__bridge void*)audioModel, (__bridge void*)placeholderInfo);
                ((void (*)(id, SEL, id, id, id, void (^)(BOOL), void (^)(int, NSString *)))objc_msgSend)
                    (handler, sendSel, audioModel, placeholderInfo, nil, (void (^)(BOOL))nil, (void (^)(int, NSString *))nil);
                TTLog(@"[qq] sendPttMsg 已调用 (ph=真实) — 等待消息出现");
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.send.enabled = YES; [self.spinner stopAnimating];
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.1 init (dlopen显式句柄 + 四参桥 + 真实placeholderInfo)");
