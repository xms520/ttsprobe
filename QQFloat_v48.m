/*
 * QQFloat_v48.m — v4.8: v4.7 日志判决（QQFloat.log 5978B）→ 语音仍 1 秒/静音:
 *   ✅ 输入/发送全链正常: PCM maxAbs=12164/10097(有真实语音, 非静音输入)
 *   ✅ sendResult code=0 全链闭环(发送成功, 只是声音是静音/1秒)
 *   ❌ 真凶实锤: encode:withSamplesCount: 的 count 参数语义 = 总样本数, 必须一次性喂入!
 *      v4.7 把 PCM 切成 20ms(320样本)×107 帧逐帧 encode → 每帧孤立样本只产出
 *      2 字节 CNG 静音帧头 → 铁证:
 *        cbs=214 = 107帧×2, nMax=214, dump 从 n=6 起恒 29f752f6a121 重复
 *        body=214B≈0.8kbps 静音, 非真实录音 3824B/2s≈15kbps(每 20ms 帧≈38B)
 *   ❌ v4.6/v4.7 "候选A末次全量" 误判: 末次快照抓到的是 107 个孤立静音帧头的拼接
 *
 * v4.8 改动:
 *   1. encode 分帧循环删除 → 一次性 encode(count = 总样本数 total)
 *      callback n=累计字节数 1→完整 silk 字节数(约 15kbps×秒数), d=累计输出缓冲
 *   2. 保留 16kHz(codec 默认) + 真实头照抄 + 候选A末次快照
 *
 * v4.8 判决树:
 *   - [silk] cbs=N nMax=X body=XB, X ≈ 15kbps×秒数(如 2.1s≈4000B) → 编码正常=终审✅
 *   - X 仍 ~200B(静音) → count 语义仍不对 → v4.9 试 count=字节数(pcm.length) 或分帧但 stateful
 *   - 播放仍 1 秒 → silk 文件通但 QQ 播放端按头[11:13] 帧长读 → 看 head32B 与真实头对齐
 *   ⚠️ 副作用不变: 真实录音文件被覆写
 *
 * ====================== v4.7 判决存档 ======================
 * QQFloat_v47.m — v4.7: v4.6 日志判决 → 采样率/候选A 修正(已被 v4.8 推翻 count 语义):
 *   callback 语义: n=当前输出缓冲累计字节数, d=缓冲指针(正确)
 *   encode 签名 Q (@ Q @?) = (id, SEL, @data, Q count, @?block)
 *   ❌ 误判: "16kHz 喂 24kHz codec" 错 — 真因是分帧喂孤立样本触发 CNG 静音
 *
 * ====================== v4.6 判决存档 ======================
    ...
    @try {
        ...
        ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
            (codec, encSel, allSamples, total, ^(const void *d, uint64_t n) {
                ...
                if (n > nMax) nMax = n;
                /* 每次 callback: d=累计输出缓冲, n=当前累计字节数
                 * → 末次快照 = 完整 silk 输出 */
                [body setData:[NSData dataWithBytes:d length:(NSUInteger)n]];
                cbs++;
            });
        if (cbs == 0) { TTLog(@"[silk] 无帧输出"); return nil; }
        TTLog(@"[silk] cbs=%lu nMax=%llu body=%luB dump=%@",
              (unsigned long)cbs, (unsigned long long)nMax,
              (unsigned long)body.length, dumpHead);
