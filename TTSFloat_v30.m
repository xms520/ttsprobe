...
/* ==================== v28: 当前聊天自动识别（免捕捉核心） ====================
 * 遍历窗口 VC 树找聊天页（类名含 MsgContentViewController / ChatRoom），
...
static NSString *TTSCurrentChatPeer(void) {
    ...
    NSString *peer = TTSPeerFromChatVC(vc);
    if (peer.length) {
        lastHit = peer;
        return peer;
    }
...
- (void)v29_finishRecordAndWait:(int)waitedMs;
@end
...
- (void)sendDirect {
    ...
    /* v28: ① 当前聊天自动识别（最高优先级——换对话不用再捕捉） */
    NSString *autoPeer = TTSCurrentChatPeer();
    if (autoPeer.length) {
        if (![autoPeer isEqualToString:(peer ?: @"")]) {
            TTLog(@"[chat] 自动识别当前聊天: %@（旧:%@）", autoPeer, peer ?: @"-");
        }
        peer = autoPeer;
    }
    /* ② 内存没有 → 持久化恢复 */
    ...
    if (!userInfo)      userInfo = TTSLoadSessionInfo();
    BOOL restored = NO;
    if (userInfo && ![userInfo isKindOfClass:[NSDictionary class]]) userInfo = nil;
    if (userInfo) restored = YES;
    if (!peer.length)   { self.statusLabel.text = @"先按住说话一次（捕获会话）"; return; }
    if (!myWxid.length)  { self.statusLabel.text = @"先按住说话一次（捕获身份）"; return; }
    /* v29: 首次发送不再要求捕捉——AudioSender 是懒创建的，没录过音就没有实例。
     * 这里主动新建一个（微信自己的类，alloc/init 标准生命周期）。
     * ⚠️ 若微信要求 init 后的额外上下文，StartRecordFrom 会 ret=0，回落提示捕捉，不崩。 */
    if (!audioSender) {
        Class senderCls = NSClassFromString(TTSCls(0));
        if (senderCls) {
            @try {
                id fresh = [[senderCls alloc] init];
                if (fresh) {
                    @synchronized([NSObject class]) { g_audioSender = fresh; }
                    audioSender = fresh;
                    TTLog(@"[chat] AudioSender 现场创建 %p（免捕捉首次发送）", (__bridge void *)fresh);
                }
            } @catch (NSException *e) {
                TTLog(@"[chat] AudioSender 创建失败: %@", e);
            }
        }
    }
    if (!audioSender)   { self.statusLabel.text = @"发送器未就绪"; return; }
    if (restored) TTLog(@"[panel] 会话参数由持久化恢复 to=%@", peer);
    ...
    TTLog(@"[tts] backend=%ld v=%@ id=%@", (long)g_backend, voice, TTSVoiceIDForName(voice));

    /* v30: 后端分派 —— 千问直连 / 原接口，回调统一走同一后续管线 */
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            if (recording) {
                ...
                dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self v29_finishRecordAndWait:waitedCopy];
                    });
                });
            } else {
...
- (void)v29_finishRecordAndWait:(int)waitedMs {
    ...
    if (!audioSender) { TTLog(@"[v27] 无 AudioSender"); return; }
    ...
    @try {
        ...
        TTLog(@"[v27] stop done (waited=%dms) end=%d", waitedMs, seenBefore);
    } @catch (NSException *e) { TTLog(@"[v27] StopRecord 异常: %@", e); }
...
static void TTSShowBall(void) {
    ...
    if (![NSThread isMainThread]) {   /* v27: 只在主线程建窗口 */
        dispatch_async(dispatch_get_main_queue(), ^{ TTSShowBall(); });
        return;
    }
    if (![UIApplication sharedApplication].keyWindow) return;   /* v27: App 窗口未就绪不建 */
    ...
    TTLog(@"===== v29 ball shown（整块填满 + 真实链自动发送，不再回传 userData） =====");
}

@interface TTSBootstrap : NSObject
@end
@implementation TTSBootstrap
/* v27: 不再用 +load + dispatch_after 5s 挂主队列——部分机型微信启动 5s 内进后台/竞争 runloop，
 * block 照样被调度 → 在 App 自己的 window 还没建好时建 keyWindow = 冷启动闪退（那台机器的 .ips）。
 * 改为：等 App 完成启动（didFinishLaunching 的通知）→ 主线程延迟 3s → 先弹球，hook 全部 lazy。 */
+ (void)load {
    __block id obs = nil;
    obs = [[NSNotificationCenter defaultCenter]
        ...
        usingBlock:^(NSNotification *n) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)),
                           dispatch_get_main_queue(), ^{
                if (![UIApplication sharedApplication].keyWindow) return;  /* App window 未就绪不弹 */
                TTSShowBall();
                InstallPrepareSendCapture();
                InstallStartRecordObserver();
                InstallAudioQueueHook();
                TTSInitVoicesIfNeeded();
                InstallRecorderEndCapture();
...
static void TTSFloatV14Init(void) {
    ...
    QwenLoadState();   /* v30: 恢复后端/千问音色/语速/语气 */
    TTLog(@"v30 init 双后端: %@ 千问音色=%@ 语速=%.2f 语气=%@",
          g_backend == 1 ? @"千问直连" : @"原接口",
          g_qwenVoice, g_qwenRate, g_qwenInstr.length ? g_qwenInstr : @"(无)");
