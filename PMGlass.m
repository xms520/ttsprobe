...
static void install_beat(void) {
    ...
    int n_w = snprintf(rs, sizeof(rs),
        ...
        "        elseif line:find('^onehit=2$', 1, false) then st.onehit = 2\n"
        ...
        "      end\n"
                        ...
                        "      -- v30 probe4：限长记录层(sc_diag3) + 完整明文日志dump(sc_raw_full.log) + 作弊状态关联\n"
        "      if not rawget(_G, '__PM_PROBE4__') then\n"
        "        rawset(_G, '__PM_PROBE4__', true)\n"
        "        local P3LOG = home .. '/Documents/sc_diag3.txt'\n"
        "        local P3RAW = home .. '/Documents/sc_raw_full.log'\n"
        "        local p3seq = 0\n"
        "        local p3cnt = {}\n"
        "        local UNPACK = unpack\n"
        "        if type(UNPACK) ~= 'function' then UNPACK = rawget(table, 'unpack') end\n"
        "        local P3FULL = {compress=true}\n"
        "        local P3CAP  = {ondie=60, endbattle=60}\n"
        "        local function p3state()\n"
        "          local s = rawget(_G, '__PM_S__')\n"
        "          if not s then return 'state=nil' end\n"
        "          return 'god='..tostring(s.god)..' onehit='..tostring(s.onehit)..' mult='..tostring(s.mult)\n"
        "        end\n"
        "        local function p3shallow(v, d, cap)\n"
        "          cap = cap or 40\n"
        "          local t = type(v)\n"
        "          if t ~= 'table' then\n"
        "            if t == 'string' then\n"
        "              local s = v\n"
        "              local extra = ''\n"
        "              if #s > 200 then s = s:sub(1,200) extra = '...(len='..#v..')' end\n"
        "              return '\"'..s:gsub('[%%c]','?')..'\"'..extra\n"
        "            end\n"
        "            return t..':'..tostring(v)\n"
        "          end\n"
        "          if d and d > 0 then return '{...}' end\n"
        "          local parts, c = {}, 0\n"
        "          for k, vv in pairs(v) do\n"
        "            c = c + 1\n"
        "            if c <= cap then parts[#parts+1] = tostring(k)..'='..p3shallow(vv, 1, cap) end\n"
        "          end\n"
        "          if c > cap then parts[#parts+1] = '...total='..c end\n"
        "          return '{'..table.concat(parts, ',')..'}'\n"
        "        end\n"
        "        local function p3append(line)\n"
        "          local okf, lf = pcall(io.open, P3LOG, 'a')\n"
        "          if okf and lf then lf:write(line) lf:close() end\n"
        "        end\n"
        "        local function p3rawdump(tag, arg, idx)\n"
        "          local okf, rf = pcall(io.open, P3RAW, 'a')\n"
        "          if not (okf and rf) then return end\n"
        "          local first = (arg:match('^%%S+')) or '?'\n"
        "          rf:write('#### RAW-DUMP '..tag..' arg#'..tostring(idx)..' seq='..tostring(p3seq)..' state=['..p3state()..'] first=['..first..'] bytes='..tostring(#arg)..'\\n')\n"
        "          rf:write('---BEGIN---\\n')\n"
        "          rf:write(arg)\n"
        "          rf:write('\\n---END---\\n')\n"
        "          rf:close()\n"
        "        end\n"
        "        local function p3mk(tag)\n"
        "          return function(old)\n"
        "            local function wrap(...)\n"
        "              local a = {...}\n"
        "              p3seq = p3seq + 1\n"
        "              p3cnt[tag] = (p3cnt[tag] or 0) + 1\n"
        "              if p3cnt[tag] == 201 then p3append(tag..' : call cap 200 reached\\n') end\n"
        "              if P3FULL[tag] then\n"
        "                for i=1,#a do\n"
        "                  if type(a[i])=='string' then p3rawdump(tag, a[i], i) end\n"
        "                end\n"
        "              end\n"
        "              local logit = p3cnt[tag] <= 200\n"
        "              if logit then\n"
        "                p3append('=== '..tag..'#'..p3seq..' in('..#a..') ===\\n')\n"
        "                for i=1,#a do p3append('  a'..i..' = '..p3shallow(a[i],0,P3CAP[tag])..'\\n') end\n"
        "              end\n"
        ...
        "                error(outs[2], 0)\n"
        "              end\n"
        "              if logit then p3append(tag..' out = '..p3shallow(outs[2],0,P3CAP[tag])..' nret='..(#outs-1)..'\\n===\\n') end\n"
        "              return UNPACK(outs, 2, #outs)\n"
        "            end\n"
        "            return wrap\n"
        "          end\n"
        "        end\n"
        "        local p3factory = p3mk\n"
        "        local function p3wraptl(name, tag)\n"
        "          local old = rawget(ed, name)\n"
        "          if type(old) == 'function' then\n"
        "            local wok, werr = pcall(rawset, ed, name, p3factory(tag)(old))\n"
        "            if wok then p3append('wrap ok: '..name..'\\n')\n"
        "            else p3append('wraptl ERR: '..name..' '..tostring(werr)..'\\n') end\n"
        "          else\n"
        "            p3append('wrap skip: '..name..' ('..type(old)..')\\n')\n"
        "          end\n"
        "        end\n"
        "        local function p3wrapcm(cls, key, tag)\n"
        "          local C = rawget(ed, cls)\n"
        "          if type(C) ~= 'table' then p3append('wrap skip: '..cls..' missing\\n') return end\n"
        "          local old = C[key]\n"
        "          if type(old) == 'function' then\n"
        "            local wok, werr = pcall(rawset, C, key, p3factory(tag)(old))\n"
        "            if wok then p3append('wrap ok: '..cls..'.'..key..'\\n')\n"
        "            else p3append('wrapcm ERR: '..cls..'.'..key..' '..tostring(werr)..'\\n') end\n"
        "          else\n"
        "            p3append('wrap skip: '..cls..'.'..key..'\\n')\n"
        "          end\n"
        "        end\n"
        "        local function p3wrapinst(instname, subkey, mkey, tag)\n"
        "          local oki, I = pcall(rawget, ed, instname)\n"
        "          I = oki and I or nil\n"
        "          if type(I) ~= 'table' then p3append('inst skip: '..instname..'\\n') return end\n"
        "          local oki2, sub = pcall(rawget, I, subkey)\n"
        "          sub = oki2 and sub or nil\n"
        "          if type(sub) ~= 'table' then p3append('inst skip: '..instname..'.'..subkey..'\\n') return end\n"
        "          local okv, old = pcall(function() return sub[mkey] end)\n"
        "          old = okv and old or nil\n"
        "          if type(old) == 'function' then\n"
        "            pcall(rawset, sub, mkey, p3factory(tag)(old))\n"
        "            p3append('inst wrap ok: '..instname..'.'..subkey..'.'..mkey..'\\n')\n"
        "          else\n"
        "            p3append('inst wrap skip: '..instname..'.'..subkey..'.'..mkey..'\\n')\n"
        "          end\n"
        "        end\n"
        "        p3append('---- probe4 install ----\\n')\n"
        "        if UNPACK then\n"
        "          p3wraptl('CompressBattleLog','compress')\n"
        "          p3wraptl('UncompressBattleLog','uncompress')\n"
        "          p3wraptl('TryReportBattleLog','tryreport')\n"
        "          p3wrapcm('BattleRecordNet','ReportClientRawResult','report')\n"
        ...
        "          p3append('---- probe4 installed, full raw log -> sc_raw_full.log ----\\n')\n"
        "        else\n"
        "          p3append('probe4 DISABLED: unpack unavailable, no wrappers installed\\n')\n"
        "        end\n"
        "      end\n"
...
- (void)pm_hitTap:(id)sender {
    (void)sender;
    f_onehit = (f_onehit + 1) % 3;   // 关 → 温和(×1000) → 暴力(9e15) → 关（纯三态，与拉条互不相干）
    pm_write_flags();
