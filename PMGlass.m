...
static void install_beat(void) {
    ...
    int n_w = snprintf(rs, sizeof(rs),
        ...
        "      end\n"
                ...
                "      -- v28 probe3：零改动记录层 → sc_diag3.txt（只记录参数/返回，绝不影响调用链）\n"
        "      if not rawget(_G, '__PM_PROBE3__') then\n"
        ...
        "            c = c + 1\n"
        "            if c <= 40 then parts[#parts + 1] = tostring(k) .. '=' .. p3shallow(vv, 1) end\n"
        "          end\n"
        "          if c > 40 then parts[#parts + 1] = '...total=' .. c end\n"
        ...
        "        end\n"
        "        -- 工厂：mk(tag)(old) 返回包装器；安装时绝不执行 old\n"
        ...
        "          end\n"
        "        end\n"
        "        -- 活实例影子（净实例方法可能是 Ctor 拷贝，类表包装够不着时兜底）\n"
        ...
        "        else\n"
        "          p3append('probe3 DISABLED: unpack unavailable, no wrappers installed\\n')\n"
        "        end\n"
        "      end\n"
