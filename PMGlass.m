...
static void install_beat(void) {
    ...
    int n_w = snprintf(rs, sizeof(rs),
        ...
        "      -- v27 probe2：dump 校验/上报链函数类型与字段 -> sc_diag2.txt（一次性）\n"
        "      if not rawget(_G, '__PM_PROBE2__') then\n"
        "        rawset(_G, '__PM_PROBE2__', true)\n"
        "        local pok, p2f = pcall(io.open, home..'/Documents/sc_diag2.txt', 'w')\n"
        "        if pok and p2f then\n"
        "          local names = {'TryReportBattleLog','CompressBattleLog','UncompressBattleLog','BattleEndCheckSystem','battleCheckVersion','BattleRecordModel','BattleRecordController','BattleRecordNet','ReplayBattle','BattleConfigUtils','ActSettleController','ActSettleConfig','EventSettle','traceSettle','BtCheckDmg'}\n"
        "          p2f:write('== check/report fn types ==\\n')\n"
        "          for _, n in ipairs(names) do\n"
        "            local okv, rv = pcall(function() return ed[n] end)\n"
        "            local v = okv and rv or nil\n"
        "            p2f:write(n .. ' : ' .. type(v))\n"
        "            if type(v) == 'number' or type(v) == 'boolean' or type(v) == 'string' then\n"
        "              p2f:write(' = ' .. tostring(v))\n"
        "            end\n"
        "            p2f:write('\\n')\n"
        ...
        "              for k in pairs(v) do\n"
        "                cnt = cnt + 1\n"
        "                if cnt <= 25 then p2f:write('   . ' .. tostring(k) .. '\\n') end\n"
        "              end\n"
        "              if cnt > 25 then p2f:write('   ... total ' .. cnt .. '\\n') end\n"
        "            end\n"
        "          end\n"
        "          p2f:write('== check/dmg keys in candidate cfg tables ==\\n')\n"
        "          local cands = {'BattleConfigUtils','ActSettleConfig','BattleRecordModel','BattleEndCheckSystem','BattleConfig','SwitchBattleSkillBlockConfig','TrapBattleConfig','BattleRecordController'}\n"
        "          local found = 0\n"
        "          for _, cn in ipairs(cands) do\n"
        "            local okc, cv = pcall(function() return ed[cn] end)\n"
        "            cv = okc and cv or nil\n"
        "            if type(cv) == 'table' then\n"
        "              for k, vv in pairs(cv) do\n"
        "                local ks = tostring(k):lower()\n"
        "                if ks:find('check') or ks:find('dmg') or ks:find('verify') or ks:find('hash') or ks:find('version') then\n"
        "                  found = found + 1\n"
        "                  p2f:write('ed.' .. cn .. '[' .. tostring(k) .. '] = ' .. tostring(vv) .. '\\n')\n"
        "                end\n"
        "              end\n"
        "            end\n"
        "          end\n"
        "          if found == 0 then p2f:write('no check/dmg/verify/hash/version key found\\n') end\n"
        "          p2f:close()\n"
        "        end\n"
        "      end\n"
