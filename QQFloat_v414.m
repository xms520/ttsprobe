/*
 * QQFloat_v414.m — v4.14: v4.13 日志判决（QQFloat_6.log 38308B）→ "越做越回去":
 *   ❌ v4.13 一次性喂全量(count=29557) → cbs=0 callback 一次都不触发!
 *   对比: v4.8 分帧喂(count=320) → cbs=214 callback 正常触发(但 6B 静音因没设码率)
 *   ✅ 根因: callback 版 count 语义 = 每帧样本数(320), 必须分帧喂!
 *      v4.8→v4.13 之间我误把"分帧"改成"一次性", 导致 count 超界 cbs=0
 *   正确组合 = 分帧(320) + 设码率(setSilkBitRate:24000) — 之前从没同时做过!
 *
 * v4.14 改动:
 *   1. 分帧循环(每 320 样本一次 encode callback 版, count=320)
 *   2. 每帧 callback 末次快照拼接成完整 silk + 码率参数已设
 *
 * v4.14 判决树:
 *   - [silk] 分帧 callback: cbs=N nFrames=M totalOut=XB, X ≈ 16kbps×秒数 → 终审✅
 *   - dump=[帧0=~40b ...] 非 6b 静音 → 真实语音帧(码率参数生效)
 *   - ⚠️ 副作用不变: 真实录音文件被覆写
 *
 * ====================== v4.13 判决存档 ======================
 * QQFloat_v413.m — v4.13: 一次性 callback 版喂全量 → cbs=0(误判 count=总样本数)
 *
 * ====================== v4.6 判决存档 ======================
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        ...
        /* v4.14: 分帧(count=320) + callback 版 + 正确码率参数 — 唯一没试过的正确组合!
         * v4.13 铁证: 一次性喂全量(count=29557) → cbs=0 callback 一次都不触发!
         *   对比 v4.8 分帧喂(count=320) → cbs=214 callback 正常触发(但 6B 静音因没设码率)
         * 结论: callback 版 count 语义 = 每帧样本数(320), 必须分帧喂!
         * 正确组合 = 分帧(320) + 设码率(setSilkBitRate:24000) = 之前从没同时做过 */
        ...
        __block NSMutableData *silk = [NSMutableData data];  /* 所有帧的完整输出拼接 */
        __block NSUInteger cbs = 0, nFrames = 0, totalOut = 0;
        __block NSMutableString *dumpHead = [NSMutableString string];
        /* 分帧喂: 每帧 320 样本, count=320, callback 累积每帧的完整输出 */
        for (NSUInteger off = 0; off + frame <= total; off += frame) {
            @autoreleasepool {
                NSData *chunk = [NSData dataWithBytes:samples+off length:frame*2];
                __block NSMutableData *frameBody = [NSMutableData data];
                __block uint64_t frameMax = 0;
                ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
                    (codec, cbSel, chunk, (NSUInteger)frame, ^(const void *d, uint64_t n) {
                        ...
                        [frameBody setData:[NSData dataWithBytes:d length:(NSUInteger)n]];
                        cbs++;
                    });
                if (frameBody.length > 0) {
                    [silk appendData:frameBody];
                    totalOut += frameBody.length;
                    if (nFrames < 3) {
                        const unsigned char *fp = frameBody.bytes;
                        NSMutableString *hx = [NSMutableString string];
                        for (NSUInteger k = 0; k < frameBody.length && k < 16; k++)
                            [hx appendFormat:@"%02x", fp[k]];
                        [dumpHead appendFormat:@"[帧%lu=%lub %@]", (unsigned long)nFrames,
                              (unsigned long)frameBody.length, hx];
                    }
                    nFrames++;
                }
            }
        }
        if (nFrames == 0 || silk.length == 0) {
            TTLog(@"[silk] 分帧 callback 无输出 (cbs=%lu nFrames=%lu total=%lu frame=%lu)",
                  (unsigned long)cbs, (unsigned long)nFrames, (unsigned long)total, (unsigned long)frame);
            return nil;
        }
        TTLog(@"[silk] 分帧 callback: cbs=%lu nFrames=%lu totalOut=%luB (%.1f kbps) dump=%s",
              (unsigned long)cbs, (unsigned long)nFrames, (unsigned long)totalOut,
              (double)totalOut * 8 / ((double)total / rate), dumpHead.UTF8String);
...
static void QQFloatV2Init(void) {
    ...
    TTLog(@"QQFloat v4.14 init (分帧320 + callback + 码率24000 正确组合)");
