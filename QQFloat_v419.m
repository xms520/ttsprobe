...
static uint64_t QQEncodeHook(id self, SEL _cmd, id data, uint64_t count, id cb) {
    if (!g_ttsEncoding && g_hookLogs < 4) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            TTLog(@"[codec-hook] self=%p count=%llu len=%lu head=%s",
                  (__bridge void *)sself, c, (unsigned long)l, g_realHeadHex);
        });
    }
    if (g_origEncodeIMP)
...
/* 分帧编码辅助: 每帧 [u16LE len][frame] 打包, 返回 voice 帧数(>4B) */
static NSUInteger SilkEncodeFrames(id codec, NSData *pcm, uint32_t rate,
    ...
    if (!codec || ![codec respondsToSelector:cbSel]) return 0;
    ...
    for (NSUInteger off = 0; off + frame <= total && nF < maxFrames; off += frame) {
        @autoreleasepool {
            ...
            ((uint64_t (*)(id, SEL, id, NSUInteger, void (^)(const void *, uint64_t)))objc_msgSend)
                (codec, cbSel, chunk, (NSUInteger)frame, ^(const void *d, uint64_t n) {
                    if (!d || !n) return;
                    lastD = d; lastN = n; cbs++;
                });
            if (lastN > before && lastD) {
                uint16_t flen = (uint16_t)(lastN - before);
                [outPacked appendBytes:&flen length:2];
...
static NSData *QQSilkEncode(NSData *pcm, uint32_t rate) {
    ...
    @try {
        /* v4.18: v4.17 判决 — PCM 完全正常(rms=2468, |v|>1000 占46%) 但 A/B/C/D 四路全同 voice=0
         * → [[QQSilkCodec alloc] init] 的实例从未进入工作状态(全 CNG 兜底)
         * 三路并行判决:
         *   REAL = hook 捕获的 QQ 真实录音 codec 实例(必然被 QQ 正确初始化)
         *   REC  = QQSilkRecorder.initEncoder 内部创建的 encoder(官方初始化路径)
         *   OWN  = 自己 alloc 的实例(对照, 必 voice=0) */
        const int16_t *samples = (const int16_t *)pcm.bytes;
        NSUInteger total = pcm.length / 2;
        {
            NSUInteger nz = 0, above = 0; long long sumSq = 0; int16_t peak = 0;
            ...
            TTLog(@"[pcm] rms=%.0f peak=%d |v|>1000:%lu%%", rms, peak, (unsigned long)(above*100/total));
        }

        NSMutableData *packR = [NSMutableData data], *packE = [NSMutableData data], *packO = [NSMutableData data];
        NSUInteger fR = 0, fE = 0, fO = 0, cR = 0, cE = 0, cO = 0;
        NSUInteger vR = 0, vE = 0, vO = 0;

        /* REAL 路: QQ 真实 codec 实例 */
        if (g_realCodec) {
            TTLog(@"[silk] REAL codec=%p (QQ真实录音捕获实例, 其真实 count=%llu)",
                  (__bridge void *)g_realCodec, g_realCount);
            g_ttsEncoding = YES;
            vR = SilkEncodeFrames(g_realCodec, pcm, rate, 20, packR, &fR, &cR);
            g_ttsEncoding = NO;
            TTLog(@"[silk] REAL(QQ实例): cbs=%lu frames=%lu voice=%lu out=%luB",
                  (unsigned long)cR, (unsigned long)fR, (unsigned long)vR, (unsigned long)packR.length);
        } else {
            TTLog(@"[silk] REAL 跳过 — 尚未捕获 QQ 真实 codec(需先录一条真实语音)");
        }

        /* REC 路: Recorder.initEncoder → encoder */
        @try {
            ...
            if (rc) {
                ...
                rec = ((id (*)(id, SEL, uint64_t, uint64_t))objc_msgSend)
                    (rec, NSSelectorFromString(@"initWithSampleRate:andBitRate:"),
                     (uint64_t)rate, (uint64_t)24000);
                if (rec) {
                    ((BOOL (*)(id, SEL))objc_msgSend)(rec, NSSelectorFromString(@"initEncoder"));
                    id renc = ((id (*)(id, SEL))objc_msgSend)(rec, NSSelectorFromString(@"encoder"));
                    TTLog(@"[silk] REC encoder=%p class=%@", (__bridge void*)renc,
                          renc ? object_getClassName(renc) : "nil");
                    if (renc) {
                        g_ttsEncoding = YES;
                        vE = SilkEncodeFrames(renc, pcm, rate, 20, packE, &fE, &cE);
                        g_ttsEncoding = NO;
                        TTLog(@"[silk] REC(Recorder.encoder): cbs=%lu frames=%lu voice=%lu out=%luB",
                              (unsigned long)cE, (unsigned long)fE, (unsigned long)vE,
                              (unsigned long)packE.length);
                    }
                } else TTLog(@"[silk] REC init 失败");
            } else TTLog(@"[silk] QQSilkRecorder MISS");
        } @catch (NSException *e) { g_ttsEncoding = NO; TTLog(@"[silk] REC 异常 %@", e); }

        /* OWN 路: 对照 */
        @try {
            id own = [[c alloc] init];
            SEL epSel = NSSelectorFromString(@"setEncodeParam");
            if ([own respondsToSelector:epSel]) ((void (*)(id, SEL))objc_msgSend)(own, epSel);
            vO = SilkEncodeFrames(own, pcm, rate, 20, packO, &fO, &cO);
            TTLog(@"[silk] OWN(alloc对照): cbs=%lu frames=%lu voice=%lu out=%luB",
                  (unsigned long)cO, (unsigned long)fO, (unsigned long)vO, (unsigned long)packO.length);
        } @catch (NSException *e) { TTLog(@"[silk] OWN 异常 %@", e); }

        /* 选路: REAL > REC > OWN, voice>0 优先, 否则输出最大 */
        NSData *best = packR; NSUInteger bi = 0;
        if (packE.length > best.length) { best = packE; bi = 1; }
        if (packO.length > best.length) { best = packO; bi = 2; }
        TTLog(@"[silk] 胜出: %s — 全量编码", bi == 0 ? "REAL" : (bi == 1 ? "REC" : "OWN"));

        /* 全量编码 */
        NSMutableData *full = [NSMutableData data];
        NSUInteger fF = 0, cF = 0;
        g_ttsEncoding = YES;
        if (bi == 0 && g_realCodec) {
            SilkEncodeFrames(g_realCodec, pcm, rate, NSUIntegerMax, full, &fF, &cF);
        } else if (bi == 1) {
            @try {
                Class rc = NSClassFromString(@"QQSilkRecorder");
                id rec = ((id (*)(id, SEL))objc_msgSend)
                ...
                ((BOOL (*)(id, SEL))objc_msgSend)(rec, NSSelectorFromString(@"initEncoder"));
                id renc = ((id (*)(id, SEL))objc_msgSend)(rec, NSSelectorFromString(@"encoder"));
                if (renc) SilkEncodeFrames(renc, pcm, rate, NSUIntegerMax, full, &fF, &cF);
            } @catch (NSException *e) { TTLog(@"[silk] REC-full 异常 %@", e); }
        } else {
            @try {
                id own = [[c alloc] init];
                SEL epSel = NSSelectorFromString(@"setEncodeParam");
                if ([own respondsToSelector:epSel]) ((void (*)(id, SEL))objc_msgSend)(own, epSel);
                SilkEncodeFrames(own, pcm, rate, NSUIntegerMax, full, &fF, &cF);
            } @catch (NSException *e) { TTLog(@"[silk] OWN-full 异常 %@", e); }
        }
        g_ttsEncoding = NO;

        if (full.length == 0) { TTLog(@"[silk] 全量编码失败"); return nil; }
        TTLog(@"[silk] FULL: cbs=%lu frames=%lu out=%luB", (unsigned long)cF, (unsigned long)fF,
              (unsigned long)full.length);

        NSMutableData *file = [NSMutableData data];
        if (g_realHeadLen >= 10) {
            [file appendBytes:g_realHead length:10];
        } else {
            [file appendBytes:"#!SILK_V3" length:9];
        }
        [file appendData:full];
        TTLog(@"[silk] silk文件共 %luB", (unsigned long)file.length);
        return file;
...
static void QQFloatV2Init(void) {
    ...
    InstallCodecHook();
    TTLog(@"QQFloat v4.19 init (修 v4.18 codec-hook ARC 返回语义闪退 + REAL/REC/OWN三路判决)");
